...
**작화 체크(개정)**: 손미림 의상은 **붙는 상의 + 통 넓은 바지**로 확정됐다(ART-RFP 2.4-2 개정). AC-PSN-06 판정도 '갖춰 입음' 지목에서 **'상·하 폭 대비' 지목**으로 개정됐다. 헤비메탈은 겉으로 드러나며 숨기지 않되, 판독 가능한 글자·로고는 2.5-6 위반이므로 가상 몬스터 도안만 쓴다. '멀쩡해 보임'은 ④ 주변 환경(플래너·15분 그리드 보드·케이블 정리)이 전담한다.
...
**판정 기준**: 이 이미지를 흑백 실루엣으로 변환했을 때, 판정단 3인 중 2인 이상이 ① 5인을 서로 구분하고 ② 4번을 "상·하 폭 대비가 가장 큰 인물"로 지목하면 합격.
