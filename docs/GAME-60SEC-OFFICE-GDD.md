...
> **"납품까지 7일. 퇴근까지 60초. 그리고 밤 9시 57분, 사무실은 더 이상 사무실이 아니다."**

| 항목 | 내용 |
|------|------|
| 제목(가칭) | 60초 오피스: D-7 (60 Seconds Office: D-7) |
| 장르 | 서바이벌 어드벤처 + 로그라이크 (택틱컬 스케빈지 & 초이스 드리븐) |
| 원안 | 「60 Seconds!」의 코어 구조(스캐빈지 → 일차별 생존)를 한국 중소기업 사무실로 재해석 |
...
핵전쟁 벙커 대신 **부도 직전의 중소기업 사무실**, 낙진 대신 **데드라인**, 바퀴벌레 대신 **복사기 요괴**. 「60 Seconds!」가 "60초 안에 챙긴 것으로 벙커에서 버티는 게임"이라면, 《60초 오피스: D-7》는 **"아침 60초 안에 챙긴 것으로 하루를 버티고, 7일 안에 빅프로젝트를 납품하는 게임"**이다.
...
3. **모두가 아는 인물들** — 대표/실장/팀장/대리라는 한국 직장의 원형적 인물 4인. 스탯 덩어리가 아니라 7일짜리 개인 서사를 가진 생존 파트너.
4. **타임루프 로그라이크** — 실패해도 '골드라벨 믹스커피'로 D-7 아침으로 회귀. 죽음(퇴사)조차 콘텐츠가 되는 메타 진행.

### 1.3 톤 앤 매너

- **낮**: K-직장 블랙코미디. 과장된 리얼리즘 (전체회신 사고, "간단한 수정", 보고서 무한 반려).
...
㈜버티고. 12년차 SI 중소기업, 직원 12명, 만년 적자. 대기업 제왕그룹의 차세대 물류 시스템 「아틀라스」 구축을 수주하며 마지막 기회를 잡았다. 납품까지 남은 시간 7일. 실패하면 부도다.

### 2.2 주인공

입사 3개월차 신입사원 **김신입**(기본명, 변경 가능). 입사 서류 더미에 섞여 있던 정체불명의 **'갑·을·병·정 계약서'**에 서명해버린 탓에, 밤 9시 57분 이후의 사무실 — **이면(裏面) 오피스** — 가 보인다. 귀신이 보이는 건 회사에서 신입뿐이다. 아무도 믿어주지 않는다.

### 2.3 이면 오피스

매일 밤 9시 57분(정시 퇴근의 미련이 완전히 죽는 시각), 사무실의 이면이 열린다. 형광등은 주파수가 바뀌고, 복사기는 스스로 출력을 시작하며, 존재하지 않는 4층으로 가는 엘리베이터가 도착음을 울린다. 이면 오피스는 이 건물을 거쳐간 모든 야근과 퇴사의 잔류물이 고인 곳 — 그 가장 깊은 곳, 지하 서버실에는 1997년 IMF 밤샘 중 사라진 전설의 개발자 **'정전무'**가 아직도 무언가를 컴파일하고 있다.

### 2.4 타임루프

납품에 실패한 날 밤, 탕비실의 낡은 자판기가 아무도 누르지 않은 버튼으로 **'골드라벨 믹스커피'**를 뱉어낸다. 그것을 마시면 D-7 아침, 출근 지하철 안에서 눈을 뜬다. 루프를 기억하는 것은 김신입뿐이다. 이 루프의 정체 역시 게임의 최심부 떡밥이다.
...
| 2 | 세계관 & 시놉시스 | ㈜버티고, 이면 오피스, 타임루프 |
| 3 | 등장인물 | 5인 프로필·스탯·시너지·7일 개인 서사·전용 대사 |
| 4 | 코어 루프 & 시스템 (정본) | 자원·스크램블·업무 페이즈·상태이상 12종·D-0 납품 판정·난이도 |
...
본 섹션은 플레이어블 캐릭터 1인과 핵심 NPC 4인의 프로필, 게임플레이 수치, 관계 시스템, 개인 서사를 정의한다. 모든 NPC는 **개인 유대도(0~100, 초기값 20)** 를 가지며, 이벤트 선택지로 증감한다(팀 공유 자원인 '대표 신뢰도'·'클라이언트 신뢰도'와는 별개의 인물별 수치다). 유대도 30/60/90 도달 시 개인 이벤트 체인이 1단계씩 해금된다. 멘탈 0·체력 0의 붕괴 처리는 시스템 섹션의 **'멘탈 0 / 체력 0 붕괴 처리 표'** 를 단일 정본으로 따른다 — 단 김신입(플레이어)의 체력 0은 병가가 아니라 즉시 ED-09 배드엔딩이다.

#### 4.1 프로필 카드

| 인물 | 나이/직급 | 한 줄 소개 | 성격 | 두려워하는 것 | 숨겨진 비밀 (공개 시점) |
|---|---|---|---|---|---|
| **김신입** (플레이어) | 27 / 사원 (입사 3개월) | 갑·을·병·정 계약서에 서명한 죄로 밤의 사무실을 보게 된 막내 | 눈치 빠르고 성실하나 거절을 못 함. 위기 속에서 의외의 배짱 | 아무도 자기 말을 믿어주지 않는 것. 그리고 정규직 전환 실패 | 계약서의 갑·을·병 서명자는 차만수·백은영·정전무. 김신입은 비어 있던 마지막 '정(丁)'을 채운 네 번째 서명자다 (D-1) |
| **차만수** 대표 | 55 / 대표이사 | 맨손으로 12년을 버틴 자수성가 사장, 오늘의 기분이 곧 사규 | 카리스마와 변덕의 시계추. 정에 약하고 숫자에 약함 | 1997년이 반복되는 것. 부도 도장 소리 | 1997년 IMF 밤샘 당시 정전무와 같은 사무실에 있었고, 그날 밤 혼자 걸어 나온 생존자다 (D-3) |
| **백은영** 실장 | 45 / 경영지원실장 | 적자 회사를 예산의 힘으로 12년 연명시킨 냉혈 살림꾼 | 감정을 결재판 뒤에 숨김. 계산이 끝나기 전엔 움직이지 않음 | 통제 불능. 자기 계산이 틀렸다는 증거 | 제왕그룹으로부터 "납품 실패 시 회사를 헐값 인수"하는 제안을 받고 홀로 저울질 중이다 (D-2) |
| **구본철** 팀장 | 41 / 개발팀장 | 회사에서 가장 늦게 불을 끄는 야근의 화신 | 책임감이 인격을 초과함. 부하에겐 무르고 자신에겐 잔인 | 텅 빈 집. 울리지 않는 휴대폰 | '집과 회사 사이에서 부서지는 중'이 아니다 — 집은 이미 비었다. 3개월째 별거 중이며 차에서 잔다 (D-5) |
| **한지우** 대리 | 29 / 선임 개발자 | 이 회사 진척도의 절반을 혼자 만드는 번아웃 직전의 에이스 | 유능하고 시니컬. 다정함을 들키면 화를 냄 | 이 회사에서의 10년 뒤 자기 모습 — 즉, 구본철 | 이직 최종 합격한 곳이 하필 클라이언트 **제왕그룹**이며, 첫 출근일이 납품일 다음 날이다 (D-2) |
...
| 차만수 | 4 | 100 | 70 | **오너 리스크** — 같은 슬롯 동료 산출 +10%, 단 매일 20% 확률로 '변덕 이벤트' 발생(슬롯 재배치 강제) | **회장님 골프 약속**: 차만수 당일 배치 불가, 예산 +80만 원. 쿨다운 2일 |
| 백은영 | 7 | 90 | 80 | **살림의 신** — 매일 결산 시 예산 소모 총액 -10% | **결재 원큐 패스**: 본인 멘탈 -15, 당일 문서 슬롯 산출 2배. 쿨다운 2일 |
| 구본철 | 8 | 60 | 60 | **야근의 화신** — 야근 페이즈 진척도 +50%, 대신 다음 날 체력 추가 -10 | **몸빵 리더십**: 본인 체력 -20, 당일 같은 슬롯 동료의 멘탈 감소 전부 무효. 쿨다운 1일 |
| 한지우 | 10 | 50 | 70 | **에이스의 대가** — 개발 슬롯 산출 +30%, 단 매일 자동 멘탈 -5 | **한 번만 갈아넣기**: 본인 멘탈 -25, 진척도 즉시 +8%. 쿨다운 3일 |

> 설계 의도: 업무력 총합은 낮게(34 — 시스템 섹션 3.1 기초 스탯 표와 동일 수치) 잡아 "누구를 갈아 넣을 것인가"의 죄책감 루프를 만든다. 한지우는 최고 화력·최저 멘탈, 구본철은 야근 특화·최저 내구로, 강한 카드일수록 빨리 부서지도록 설계한다.
>
...
| 조합 | 관계 유형 | 서사적 이유 |
|---|---|---|
| 김신입 × 차만수 | 혼합 — 대표의 총애와 막내의 소모 | 대표는 막내를 예뻐한다. 막내는 30분짜리 라떼 강의를 견딘다 |
| 김신입 × 백은영 | 시너지 — 스파르타 성장형 | 백은영식 스파르타 멘토링. 혹독하지만 배운다 |
| 김신입 × 구본철 | 혼합 — 치유형이나 손은 느려진다 | 가르치느라 손은 느려지지만, 누군가를 키우는 일이 그를 버티게 한다 |
| 김신입 × 한지우 | 시너지 — 안정형(멘탈 보호) | 막내 앞에서만 한지우는 "이 일이 원래 재밌었다"는 걸 기억한다 |
| 차만수 × 백은영 | 조건부 시너지 — 응대 특화, 실무에선 역효과 | 12년 콤비의 영업 합은 예술. 단 실무 슬롯에선 옛날 얘기만 한다 |
| 차만수 × 구본철 | 역시너지 — 멘탈 소모형 | "요즘 개발이 뭐가 힘들어, 우리 땐 어셈블리로…" 현장 모르는 소리가 그를 부순다 |
| 차만수 × 한지우 | 역시너지 — 퇴사 리스크형 | "요즘 애들은 근성이…" 한 마디가 사직서 폴더를 연다 |
| 백은영 × 구본철 | 시너지 — 문서·QA 특화 | 말없이 서류가 오간다. 회사에서 유일하게 서로를 갈아 넣지 않는 두 사람 |
| 백은영 × 한지우 | 혼합 — 감시 가속형 | 백은영은 이직 낌새를 눈치챘다. 감시의 눈빛 아래 손은 빨라지고 속은 탄다 |
| 구본철 × 한지우 | 시너지 — 최고 효율·상호 소모 | 회사 최강의 페어이자 서로의 미래·과거를 보는 거울. 최고 효율, 최악의 침묵 |

#### 4.4 인물별 7일 스토리 아크

| D-day | 김신입 | 차만수 | 백은영 | 구본철 | 한지우 |
|---|---|---|---|---|---|
| D-7 | 서류 더미 속 계약서를 처음 자각한다(튜토리얼) | "무조건 된다"며 전 직원 앞 호언장담 | 예산 통제 선언, 법인카드 한도 축소 | 누가 시키지도 않은 야근을 자원한다 | 화장실에서 몰래 면접 일정 통화 |
| D-6 | 밤 9시 57분, 이면 오피스와 첫 조우 | 제왕그룹 임원과 골프 회동, 예산 유출 | 제왕그룹에서 온 정체불명의 전화를 받는다 | 아내의 전화를 세 번 거절한다 | 제왕그룹 최종 합격 통보를 받는다 |
| D-5 | 복사기 요괴에게서 '97년 사원명부'를 입수 | 은행에서 대출 연장 거절 통보 | 밀봉된 인수 제안서를 몰래 검토한다 | 주차장 차 안에서 이불을 덮고 자는 모습이 목격된다 | 빈 회의실에서 사직서를 쓴다 |
| D-4 | 명부에서 차만수·백은영의 이름을 발견 | 밤, 혼자 남아 97년 단체사진을 꺼내 본다 | 한지우의 이직 준비를 눈치챈다 | 서랍 속 아이의 그림이 발견된다 | 사직서 파일을 김신입에게 들킬 뻔한다 |
| D-3 | 정전무의 귀신과 첫 대화에 성공 | '정전무'라는 이름에 처음으로 손을 떤다 | 김신입에게 시험하듯 회사의 미래를 묻는다 | 과로로 쓰러질 뻔한다(체력 25 미만 시 실제 병가) | 양심의 가책으로 몰래 인수인계 문서를 만든다 |
| D-2 | 계약서 '정' 서명의 의미를 파악한다 | 사재 출연 결단 혹은 골프장 잠적(유대도 60 분기) | 인수 제안 회신 마감 — 찢거나, 서명하거나 | 유대도 60 이상이면 입사 후 첫 정시퇴근 | 백은영 앞에서 사직서 제출을 시도한다 |
| D-1 | 계약을 승계할 것인가, 파기할 것인가 — 최종 선택 | 납품 프레젠 최전선에 서거나, 자리가 빈다 | 회사를 살렸는지 팔았는지가 엔딩 분기에 반영 | 가족에게 전화를 걸거나, 최후의 밤샘 | 잔류/퇴사 선택 — 유대도 90 시 '납품만 끝내고 간다' 제3의 길 |
...
| 차만수 | 「회장님의 골프채」 — 스킬 쿨다운 -1일 | 「1997년 단체사진」 — 정전무 단서 획득 | 「그 밤의 생존자」 — D-2 사재 출연 확정, 예산 +200만 원 |
| 백은영 | 「영수증의 여왕」 — 예산 소모 추가 -5% | 「두 개의 제안서」 — 인수 제안 존재를 플레이어에게 공유 | 「살림꾼의 진심」 — D-2에 제안서를 찢는다 |
| 구본철 | 「식은 도시락」 — 야근 체력 소모 -5 | 「차 안의 이불」 — 별거 사실 고백, 멘탈 최대치 +10 | 「집으로 가는 길」 — 이탈 조건이 멘탈 0 → -20으로 완화 |
| 한지우 | 「모니터 뒤의 이력서」 — 자동 멘탈 감소 -5 → -3 | 「합격 통보」 — 이직 사실 고백 | 「마지막 인수인계」 — D-1 잔류 시 최종일 산출 2배 |
...
3. 「회식 생존기」 — 대표의 기습 회식 소집. 참석 시 대표 신뢰도 +5, 전원 체력 -15.
4. 「믹스커피 감별사」 — 자판기에서 골드라벨이 아닌 '실버라벨'이 나왔다. 마실 것인가.
5. 「사원증의 무게」 — 사원증 사진이 어제와 미묘하게 다르다. 루프의 흔적을 낮에 처음 마주한다.

**차만수**
1. 「대표님의 아이디어」 — 밤새 떠오른 신기능 추가 지시. 수락 시 진척도 -10%, 대표 신뢰도 +10.
2. 「골프 접대 대작전」 — 제왕그룹 상무와 라운딩. 예산 -60만 원, 클라이언트 악재 이벤트 확률 절반.
3. 「아침 조회수 경영」 — 갑작스런 전체 조회로 오늘 업무 슬롯 1개 봉인.
4. 「법인카드의 행방」 — 카드 실종. 60초 스크램블에서 발견 시 예산 +40만 원 회수.
5. 「IMF 이야기」 — 야근 중 시작되는 97년 회고. 끝까지 들으면 정전무 단서 +1, 체력 -10.

**백은영**
1. 「영수증 심판의 날」 — 증빙 누락 발각. 3일 내 미해결 시 예산 -30만 원.
2. 「실장님의 미소」 — 백은영이 웃었다. 이유를 캐묻거나(진실 단서), 모른 척한다(전원 멘탈 -5).
3. 「제왕그룹에서 온 봉투」 — 밀봉 제안서 전달 심부름. 열어볼 것인가.
4. 「비상금 금고」 — 유대도 60 이상 시 백은영만 아는 비상 예산 +50만 원 해금.
5. 「인사평가 시즌」 — 오늘 백은영이 지정한 배치안을 그대로 따르면 전 슬롯 산출 +15%.

**구본철**
1. 「기절 야근」 — 서버실에서 잠든 채 발견. 깨우지 않으면 야근 자동 진행, 이면 조우 확률 2배.
2. 「팀장의 서랍」 — 아이의 그림 발견. 비밀 아크 진입 열쇠.
3. 「대리 출근」 — 몸살인데 출근했다. 돌려보내면 진척도 -5%, 구본철 멘탈 +20.
4. 「라꾸라꾸 침대」 — 스크램블에서 접이식 침대 확보 시 야근 체력 소모 -50%.
5. 「퇴근하세요, 팀장님」 — 전원 합심 강제 퇴근 작전. 성공 시 다음 날 구본철 업무력 +2.

**한지우**
1. 「면접 알리바이」 — 수상한 반차 요청. 승인하면 개발 슬롯 공백, 거절하면 한지우 멘탈 -15.
2. 「에이스의 어깨」 — 서버 장애. 한지우 단독 해결(멘탈 -20) vs 전원 투입(진척도 -5%).
3. 「헤드헌터의 전화」 — 자리에 울리는 낯선 번호. 김신입이 대신 받을 것인가.
4. 「키보드 소리」 — 새벽, 아무도 없는 한지우의 자리에서 타건 소리가 들린다(이면 오피스 연계).
5. 「사직서 폴더」 — 공유 서버에 잘못 올라간 'final_진짜최종_사직서.hwp'. 본 사람은 아직 김신입뿐.
...
| 차만수 | "우리가 가족 같은 회사잖아. 어? 가족!" | "내가 IMF도 버틴 사람이야. …그때도 딱 이런 밤이었지." | (복사기 요괴의 울음을 들으며) "복합기 리스 바꾸라니까. 기계가 곡소리를 내네." |
| 백은영 | "법인카드 영수증, 오늘 안 내면 사비 처리합니다." | "예산은 거짓말을 안 해요. 사람이 하지." | (정전무가 스친 자리에서) "왜 이렇게 춥니. 관리비 아깝게 누가 창문 열었어?" |
| 구본철 | "먼저 들어가. 난 이것만 보고." | "집에 가서 뭐 하게. …아니, 가긴 갈 건데." | (4층 버튼이 생긴 엘리베이터 앞에서) "우리 건물에 4층이… 있었나. 피곤하니까 별게 다 보이네." |
| 한지우 | "그거 제가 할게요. 어차피 제가 해야 끝나요." | "제가 없으면 안 돌아가는 회사면, 그건 회사가 아니라 인질극이죠." | (퇴사자의 그림자를 지나치며) "방금 최 대리님 향수 냄새… 아니다. 그분 작년에 나갔지." |
...
|---|---|---|---|---|---|---|
| 진척도 | 팀 공유 | 0% | 120% | 개발·문서 슬롯, 야근 보너스, 이벤트 보상 | 이벤트 페널티(서버 폭파, 요구사항 번복), D-1 통합테스트 버그 차감 | D-4 아침 '중간보고'에서 0%면 대표 신뢰도 -30. D-0에 100% 미만이면 납품 판정의 진척도 게이트에서 즉시 실패 → ED-04 '부도'(5절 참조) |
...
| 예산 | 팀 공유 | 300만 원 | 1,000만 원 | 대표 신뢰도 60 이상 시 D-4에 한도 +200만 원, 이벤트 보상 | 커피·간식 구매, 회식·외주 등 이벤트 선택지 | 구매 불가 + '법카 정지' 이벤트 발생, 백은영 멘탈 -10. ※ 예산이 100만 원(시작 예산의 1/3) 미만이면 OF-10 '비용절감 캠페인 공문'이 확정 발동하고, -100만 원 이하로 떨어지면 ED-08 '법인카드 정지' 엔딩 |
| 대표 신뢰도 | 팀 공유 | 50 | 100 | 진척도 보고(D-4 중간보고), 골프 관련 선택지, 이벤트 보상 | 마감 관련 이벤트 실패, D-4 중간보고 부진, 이벤트 페널티 | 즉시 ED-06 '대표 잠적' 배드엔딩(엔딩 섹션 참조). 40 미만 구간에서는 경고 이벤트 '김신입 해고 위기'가 발생해 위험을 예고한다 |
| 클라이언트 신뢰도 | 팀 공유 | 50 | 100 | 클라이언트 응대 슬롯 성공 +5, 클라이언트 계열 이벤트(OF-01~05 등) | 응대 실패 -5, 클라이언트 콜 공석 -10, 이벤트 페널티(OF-25 담당자 교체 시 30으로 리셋) | 즉시 엔딩은 없으나 D-0 납품 판정의 신뢰도 보정이 하한(-25%p)으로 고정된다(5절 참조) |
| 멘탈 | 인물별 | 차만수 70 / 백은영 65 / 구본철 40 / 한지우 35 / 김신입 60 | 인물별 최대치(캐릭터 4.2 표 참조) | 휴식 슬롯 +15, 컵라면(IT-05), 이벤트 | 업무 -6~-10, 야근 -10, 이벤트, 상태이상 | 해당 인물 **이탈** — 처리는 1.1 '멘탈 0 / 체력 0 붕괴 처리 표'가 단일 정본 |
| 체력 | 인물별 | 차만수 60 / 백은영 70 / 구본철 45 / 한지우 55 / 김신입 65 | 인물별 최대치(캐릭터 4.2 표 참조) | 휴식 슬롯 +12, 에너지바(IT-06), 홍삼(IT-07) | 업무 -3~-8, 야근 -12, 상태이상 | **병가 1일** — 처리는 1.1 붕괴 처리 표가 단일 정본. 단 김신입은 즉시 ED-09 배드엔딩 |
...
| 붕괴 | 처리 | 상세 |
|---|---|---|
| 멘탈 0 | **이탈** | 해당 인물은 잔여 런 동안 배치 불가. 팀의 진척도 일일 상한 -20%. 이탈 연출은 인물별로 다르나(한지우=이직 통보, 구본철=무단결근, 차만수=잠적, 백은영=출근 거부) 기계적 처리는 전원 동일. 구본철은 유대도 90 특전으로 이탈 조건이 멘탈 0 → -20으로 완화된다(캐릭터 4.4 참조) |
...
사무실은 중앙 복도를 축으로 한 기본 5구역에, 평시 잠겨 있는 6번째 구역 **대표실**을 더한 총 6구역이다. 김신입은 매일 **출입문(책상 구역 앞)**에서 시작한다.
...
- **대표실 개방 규칙**: 평시 잠김. 매일 아침 25% 확률로 개방되며, 차만수 유대도 60 이상이면 개방 확률이 50%로 상승한다. 내부의 스폰 슬롯 2개는 고가치 전용 — 유물 RL-01 '대표의 행운 골프공', 루프 1회 이상 경험 시 ED-15 연계 히든 아이템 '깨끗한 사직서'(출현률 15%) 등이 이 슬롯에서만 스폰된다.
...
| IT-13 | 한지우의 이력서(출력 실수) | 1 | 사용 시 택1 — 모른 척: 한지우 멘탈 +10 / 보고: 대표 신뢰도 +5, 한지우 멘탈 -20 | 책상 10% |
| IT-14 | 안마쿠션 | 2 | 당일 휴식 슬롯 효과 +50% | 책상 20% |
| IT-15 | 빔프로젝터 리모컨 | 1 | 클라이언트 응대 실패 시 1회 재시도 | 회의실 30% |
| IT-16 | 아틀라스 계약서 사본 | 1 | '요구사항 번복' 이벤트 1회 무효화 | 회의실 25% |
| IT-17 | 대표의 골프 퍼터 | 2 | 차만수에게 전달 시 대표 신뢰도 +10 | 회의실 15% |
...
| 인물 | 업무력(WP) | 적성 배수 |
|---|---|---|
| 차만수 | 4 | 클라이언트 응대 ×1.5 (단, 20% 확률로 변덕 발동 → 해당 판정 보정 무효) |
| 백은영 | 7 | 문서 ×1.5, 클라이언트 응대 ×1.3 |
| 구본철 | 8 | 개발 ×1.2, QA ×1.2 |
| 한지우 | 10 | 개발 ×1.5, QA ×1.3 |
| 김신입 | 5 | 전 슬롯 ×1.0 |
...
| 조합 | 슬롯 | 효과 |
|---|---|---|
| 김신입 + 차만수 | 임의 | 혼합: 김신입 산출 +10%(곱연산), 김신입 멘탈 -3 추가(라떼 강의) |
| 김신입 + 백은영 | 문서·클라이언트 응대 | 김신입 산출 +15%(곱연산, 스파르타 첨삭) |
| 김신입 + 구본철 | 임의 | 혼합: 산출 합계 -5%(곱연산), 두 사람 멘탈 소모 -50%(가르치며 버틴다) |
| 김신입 + 한지우 | 임의 | 한지우 멘탈 소모 -50%, 산출 +5%(곱연산) |
| 차만수 + 백은영 | 클라이언트 응대 | 성공률 +20%p (변덕 발동 시에도 유지). 단 문서 슬롯 동반 배치 시 산출 합계 -10%(곱연산, 옛날 얘기로 손이 멈춘다) |
| 차만수 + 구본철 | 임의 | 역시너지: 구본철 멘탈 -5 추가(잔소리) |
| 차만수 + 한지우 | 임의 | 역시너지: 한지우 멘탈 -10 추가('요즘 애들' 발언) |
| 백은영 + 구본철 | 문서·QA | 산출 합계 +15%(곱연산, 말없이 서류가 오간다) |
| 백은영 + 한지우 | 임의 | 혼합: 한지우 산출 +10%(곱연산), 한지우 멘탈 -5 추가(감시 가속) |
| 구본철 + 한지우 | 개발 | 진척도 **+1.5%p 플랫 가산**(3.5 검산 예 준거. 산출 비례가 아닌 고정 가산 — 최강 페어의 보정이 인플레하지 않도록 플랫으로 캡) |
...
검산 예: 개발에 한지우(멘탈 35 → C 0.675 → 10.1%p) + 구본철(min 40 → C 0.7 → 6.7%p) + 시너지 +1.5%p(구본철+한지우 페어 플랫 가산, 3.4 표 참조) ≈ **일 18.3%**. 7일 × 평균 14.3% 필요이므로, 휴식일을 끼우면 야근 1~2회가 강제되는 압박 곡선이 설계 의도다.
...
D-0 아침 09:00, 7일의 결과물을 제왕그룹 최종 검수에 회부한다. 판정은 아래 3단계를 순서대로 수행한다. 같은 아침에 ED-07 '테스트 결과 조작' 발각 판정이 예약돼 있다면 검수보다 먼저 처리한다(엔딩 섹션 참조).

**① 진척도 게이트** — 진척도가 100% 미만이면 검수 자체가 성립하지 않는다. 즉시 실패로 처리되어 ED-04 '부도' 결산으로 직행한다. 게이트 기준치는 난이도별 '승리 요구 진척도'를 따르며(7절 난이도 표 참조), 본문 수치는 노멀(100%) 기준이다.

**② 검수 성공률 판정** — 게이트 통과 시 아래 성공률로 1회 판정한다.
...
| 보정 항목 | 출처 | 효과 |
|---|---|---|
| OF-07 '골프 동원령' A(동행) | 현실 이벤트 | 검수 성공률 +10%p |
...
| [아틀라스 원본 설계도] — GH-25 체인 완주 보상 | 이면 괴이 | **검수 자동 성공** — ② 판정을 생략한다(① 게이트는 그대로 적용) |

**③ 엔딩 분기** — 성공 시 결산형 엔딩 우선순위(ED-16 진 > ED-13 > ED-03 > ED-02 > ED-01, 엔딩 섹션 참조)에 따라 분기하고, 실패 시 ED-04 '부도' 계열로 결산한다(검수 탈락 — 상세는 엔딩 섹션 참조).
...
|---|---|---|---|---|---|
| OF-01 | 🔗 "간단한 수정인데요?" (체인 1/3) | D-7~D-5, 40%. 미발동 시 D-5에 강제 발동 | 제왕그룹 박 차장의 전화. "아 별건 아니고요, 대시보드 색만 저희 CI로. 아, 그리고 버튼 위치도 조금만." | A: 즉시 수용<br>B: 변경요청서 공문 요구<br>C: 한지우에게 몰래 토스 | A: 진척도 -5, 한지우 멘탈 -10, 클라이언트 신뢰도 +5. 익일 체인 2단계 확정<br>B: 클라이언트 신뢰도 -5. 50% 체인 종료 / 50% 2단계 발동<br>C: 진척도 -2, 한지우 멘탈 -20. OF-16 발동 확률 +20%p |
| OF-02 | 전체회신 대참사 | D-6~D-3, 20%. 당일 커피 보유 0개면 확률 2배 | "제왕 박차장 진짜 답없음ㅋㅋ" — 구본철에게 보낸다는 메일이 제왕그룹 전원 참조로 발송됐다. 회수 버튼은 헛돈다. | A: 즉시 사과 전화+정정 메일<br>B: "해킹당했습니다"<br>C: 백은영에게 SOS | A: 클라이언트 신뢰도 -10, 김신입 멘탈 -15. 다음 클라이언트 이벤트의 부정 결과 1.5배<br>B: 60% 성공 시 클라이언트 신뢰도 -3 / 40% 실패 시 클라이언트 신뢰도 -20, 진척도 -5<br>C: 예산 -30(과일바구니), 백은영 멘탈 -5, 클라이언트 신뢰도 무변동. OF-08 선택지 C 해금 |
| OF-03 | 🔗 주간보고 무한 반려 (체인 1/2) | D-6 확정 발동 | 제왕 PMO가 주간보고서를 반려했다. 사유: "폰트가 정성이 없음". 3차 반려 사유는 "느낌이 아쉬움"이었다. | A: 구본철 밤샘 재작성<br>B: 김신입이 야마 갈아엎기<br>C: 제왕 출신 첨삭 외주(백은영 인맥) | A: 구본철 체력 -20·멘탈 -10, 진척도 -3. 통과 70%(실패 시 익일 체인 2단계)<br>B: 김신입 멘탈 -10, 커피 -2. 통과 50%, 성공 시 클라이언트 신뢰도 +10<br>C: 예산 -50. 통과 95% |
| OF-04 | 갑작스러운 현장 실사 | D-4~D-2, 25% | "지금 근처라 30분 뒤 들르겠습니다." 제왕 품질팀이다. 사무실엔 어제 회식의 잔해와 게임 화면이 켜져 있다. | A: 전원 청소+연출 모드<br>B: 정공법<br>C: 구본철에게 즉석 시연 강제 | A: 금일 업무 배치 1슬롯 무효, 진척도 -4, 클라이언트 신뢰도 +10<br>B: 50% "진정성 있네요" 클라이언트 신뢰도 +15 / 50% 클라이언트 신뢰도 -15<br>C: 구본철 멘탈 -15, 클라이언트 신뢰도 +5 |
| OF-05 | 스코프 크리프 | 진척도 60% 이상, 30% | "저희 구매팀도 아틀라스 쓰고 싶다네요. 화면 몇 개만 추가하면 되죠? 계약은… 나중에 얘기하죠." | A: 수락<br>B: 2차 계약 역제안<br>C: 메일 '읽지 않음' 처리 | A: 진척도 -12, 전원 멘탈 -8. D-0 납품 판정(최종 검수) 시 클라이언트 신뢰도 +20 보정<br>B: 40% 성공: 예산 +200, 클라이언트 신뢰도 +5 / 60% 실패: 클라이언트 신뢰도 -10<br>C: 3일 내 25% 재발동, 재발동 시 A 강제 적용 |
...
| ID | 이벤트명 | 발생 조건·확률 | 상황 플레이버 | 선택지 | 결과 |
|---|---|---|---|---|---|
| OF-06 | [차만수] 갑작스러운 전사 회식 | D-6~D-2, 30%. 대표 신뢰도 70 이상이면 확률 2배 | 오후 4시 반, 차만수 대표가 상기된 얼굴로 등장. "오늘 같은 날 삼겹살에 소주 한잔 해야지! 법인카드 긁어!" 오늘 밤 야근 계획이 무너지는 소리가 들린다. | A: 끝까지 참석<br>B: 1차 후 탈출<br>C: "서버 지켜야 해서요" | A: 예산 -40, 전원 멘탈 +10·체력 -15, 금일 야근 봉쇄, 대표 신뢰도 +10<br>B: 예산 -40, 전원 멘탈 +5·체력 -5, 대표 신뢰도 -5, 야근 가능<br>C: 대표 신뢰도 -10, 진척도 +3. 단독 야근 시 이면 오피스 조우 확률 +15%p |
| OF-07 | [차만수] 골프 동원령 | D-5~D-3, 20% | "제왕 상무님이 골프 좋아하신다더라. 김신입이 캐디백 좀 들어라. 이것도 다 일이야, 일." | A: 동행<br>B: 거절<br>C: 한지우 대타 제안 | A: 김신입 체력 -20, 금일 김신입 배치 불가, 대표 신뢰도 +15. D-0 납품 판정 검수 성공률 +10%p(시스템 'D-0 납품 판정' 보정 항목)<br>B: 대표 신뢰도 -15, 진척도 +4<br>C: 한지우 멘탈 -25, 대표 신뢰도 +5. OF-16 발동 확률 +30%p |
| OF-08 | [백은영] 법인카드 내역 소환 | 누적 지출 150만원 초과 시 확정(1회) | 백은영 실장이 영수증 뭉치를 부채처럼 펼친다. "새벽 2시 곱창 8인분. 야근 인원은 4명이었는데, 나머지 4인분은 누가 드셨을까?" | A: 정직하게 실토<br>B: "정전무님이 드셨습니다"<br>C: (OF-02 C 해금 시) 은혜 갚기 | A: 예산 +20(환수), 전원 멘탈 -5. 이후 예산계 이벤트 부정 결과 20% 완화<br>B: 70% 정색: 백은영 유대도 -5, 김신입 멘탈 -10 / 30% 피식: 이면 오피스 힌트 1개 획득<br>C: 전면 무마, 변화 없음. 해금 카드 소멸 |
| OF-09 | 대표 보고용 'PPT 성형수술' | D-5·D-3 각 35% | 구본철이 속삭인다. "대표님은 3D 도형이랑 그라데이션 좋아하셔. 내용은… 어차피 안 읽으셔." | A: 밤새 PPT 마사지<br>B: 내용으로 승부 | A: 커피 -3, 김신입 멘탈 -10, 대표 신뢰도 +10<br>B: 60% "칙칙하네" 대표 신뢰도 -5 / 40% "이게 보고지!" 대표 신뢰도 +15 |
| OF-10 | 비용절감 캠페인 공문 | 예산이 시작 예산의 1/3(100만 원) 미만일 때 확정 | '이면지 의무화, 냉방 28도, 간식비 동결'. 공문 마지막 줄이 백미다. "단, 대표실은 예외로 한다." | A: 순응<br>B: 탕비실 사수 투쟁(백은영 설득) | A: 이후 간식 획득량 -1/일, 전원 멘탈 -8, 예산 +30<br>B: 50% 성공: 멘탈 무변동, 예산 -20 / 50% 실패: 전원 멘탈 -12, 예산 +30 |
...
| ID | 이벤트명 | 발생 조건·확률 | 상황 플레이버 | 선택지 | 결과 |
|---|---|---|---|---|---|
| OF-11 | 개발 서버 다운 | D-6~D-2, 25%. 전날 야근 시 +10%p | 오전 10시, 서버실에서 타는 냄새. 12년 된 서버 '늙은이'가 마지막 팬 소리를 낸다. | A: 한지우 긴급 복구<br>B: 유지보수 업체 호출<br>C: 클라우드 임시 이전 | A: 한지우 체력 -15, 진척도 -8(전날 야근했다면 -12)<br>B: 예산 -80, 진척도 -3<br>C: 예산 -120, 진척도 -2. 이후 서버 다운 이벤트 면역 |
| OF-12 | 프린터 토너의 배신 | OF-03/OF-04 발생일에 40% | 최종 보고서 100장 출력 중 34장째부터 줄무늬. 토너 잔량 표시는 어제까지 분명 '충분'이었다. | A: 토너 즉시 구매<br>B: 흔들어서 버티기<br>C: 전량 PDF 송부 | A: 예산 -25, 정상 출력<br>B: 60% 성공 / 40% 실패: 클라이언트 신뢰도 -5<br>C: 클라이언트 신뢰도 -3("종이로 줘야 보고지"), 예산 0 |
| OF-13 | 라이선스 만료 | D-5 확정 발동 | 개발툴 라이선스 만료. 결제 담당은 3개월 전 퇴사자. 갱신 안내 메일은 그의 잠긴 메일함 속에 있다. | A: 재구매<br>B: 30일 평가판 돌려막기<br>C: 오픈소스 이식 | A: 예산 -90<br>B: 예산 0. 이후 매일 15% 확률로 작업 중단(진척도 -4)<br>C: 진척도 -6(1회), 이후 안전 |
| OF-14 | 에어컨 사망과 폭염 | D-4~D-2, 20% | 실외기가 숨을 거뒀다. 실내 31도. 서버보다 사람이 먼저 다운될 판인데 관리사무소는 "다음 주에나"란다. | A: 수리 급행<br>B: 선풍기+아이스크림<br>C: 그냥 버티기 | A: 예산 -60<br>B: 예산 -15, 간식 +4, 전원 체력 -10(당일)<br>C: 전원 체력 -15·멘탈 -10, OF-11 발동 확률 +20%p |
| OF-15 | [한지우] force push 대참사 | 한지우 멘탈 30 미만인 날 35% | 번아웃 상태의 한지우가 이틀치 작업을 강제 푸시로 날렸다. 초점 잃은 눈. "…저 잠깐 옥상 좀." | A: 김신입의 몰래 백업으로 복구<br>B: 밤새 재작업<br>C: 옥상 동행, 오늘은 접기 | A: 진척도 -3, 한지우 멘탈 +15<br>B: 진척도 -8, 한지우 체력 -20, 금일 야근 강제<br>C: 진척도 -10, 한지우 멘탈 +25. OF-16 체인의 '이탈 루트' 확률 -20%p |
...
| ID | 이벤트명 | 발생 조건·확률 | 상황 플레이버 | 선택지 | 결과 |
|---|---|---|---|---|---|
| OF-16 | 🔗 [한지우] 수상한 반차 (체인 1/3) | D-6~D-4, 40%. OF-07 C 선택 시 +30%p | 한지우가 '치과 진료'로 반차를 냈다. 셔츠가 지나치게 다림질돼 있고, 가방에서 경력기술서가 삐져나와 있다. | A: 모른 척<br>B: 조용히 물어보기<br>C: 구본철에게 보고 | A: 진척도 -4. D-2에 체인 2단계 예약<br>B: 진척도 -4, 한지우 멘탈 +10. 체인 2단계 특수 선택지 해금<br>C: 진척도 -6, 한지우 멘탈 -20, 구본철 멘탈 -10. 체인 즉시 2단계(발각 루트) |
| OF-17 | [구본철] 학예회와 배포일 | D-3 확정 발동 | 구본철의 배경화면 속 딸이 오늘 학예회 주인공이다. 하필 통합 테스트 날. 그는 사원증 목걸이만 만지작거린다. | A: 보내주고 팀이 커버<br>B: "가족보다 회사지"<br>C: 오전만 참석 절충 | A: 진척도 -5, 구본철 멘탈 +30, 잔류 인원 체력 -10. 최종일 구본철 체력 +20 보정<br>B: 진척도 +5, 구본철 멘탈 -35. D-1 구본철 무단결근 확률 30%<br>C: 진척도 -2, 구본철 멘탈 +10 |
...
| OF-19 | [백은영×구본철] 공로 전쟁 | 진척도 50% 돌파 익일 확정 | 진척 보고 메일의 참조 순서를 두고 두 사람이 냉전에 돌입했다. 사무실 공기가 이면 오피스보다 차갑다. | A: 백은영 편<br>B: 구본철 편<br>C: "두 분 다 참조 1번으로" | A: 백은영 멘탈 +10, 구본철 멘탈 -15, 대표 신뢰도 +5. 예산계 이벤트 완화<br>B: 구본철 멘탈 +15, 백은영 멘탈 -10, 진척도 +3<br>C: 양측 멘탈 +5, 김신입 멘탈 -10(양쪽 눈치) |
| OF-20 | 방치된 신입의 서러움 | D-6~D-5, 30% | 아무도 김신입에게 사내 위키 계정을 안 만들어줬다. 점심 단톡방 초대도 없다. 귀신은 보이는데, 회식 공지는 안 보인다. | A: 밈 섞은 자기소개 전체메일<br>B: 조용히 감내<br>C: 구본철에게 정식 요청 | A: 60%: 전원 멘탈 +5, 사내 이벤트 긍정 보정 / 40%: OF-02 발동 확률 +15%p<br>B: 김신입 멘탈 -10, 커피 +2(한지우가 몰래 놓고 감). 한지우 관련 선택지 해금<br>C: 구본철 체력 -5. 온보딩 완료: 김신입 배치 효율 +10% |
...
| ID | 이벤트명 | 발생 조건·확률 | 상황 플레이버 | 선택지 | 결과 |
|---|---|---|---|---|---|
| OF-21 | 🔗 경쟁사 스카우트 전화 (분기) | 진척도 40% 이상, D-5~D-4, 20% | 모르는 번호. "김신입님이시죠? 연봉 1.5배에 야근이 없습니다. 그리고… 귀신도 없고요." 마지막 말은 환청이었기를. | A: 단칼 거절<br>B: "일단 얘기나…"(제안 수락)<br>C: 한지우에게 번호 토스 | A: 김신입 멘탈 +5. 엔딩 분기 '의리' 플래그 획득<br>B: 김신입 멘탈 +10, 아이템 '포트폴리오 조각' 1개 획득(ED-14 연계, 하단 분기 상세)<br>C: 한지우 멘탈 +15. OF-16 체인 즉시 2단계로 가속<br>※ B는 15% 확률로 사내 발각: 대표 신뢰도 -20. A·B 선택 시 D-3 저녁 분기 카드 '옥상의 이직 상담' 예약(하단 상세) |
| OF-22 | 🔗 국세청 세무조사 (체인 1/2) | D-5~D-4, 15%. OF-08에서 B 선택 시 +10%p | 정장 차림 두 사람이 들어선다. "국세청에서 나왔습니다." 차만수 대표가 골프 스윙 자세 그대로 얼어붙었다. | A: 백은영 전권 위임<br>B: 전 직원 자료 대응 | A: 백은영 2일간 배치 불가·멘탈 -20. 체인 2단계 무혐의 확률 85%<br>B: 금일 진척도 획득 0(업무 마비), 전원 멘탈 -10. 무혐의 확률 60% |
...
| OF-24 | 외주사 '코드나라' 잠적 | D-5~D-3, 20% | 결제 모듈 외주사의 연락이 끊겼다. 대표 번호는 없는 번호, 사무실엔 임대 딱지. 마지막 메일 제목은 "죄송합니다"였다. | A: 내재화(한지우+김신입)<br>B: 대체 외주 급구<br>C: 내용증명+미완성 코드 인수 | A: 진척도 -10, 한지우 체력 -20. 이후 안전<br>B: 예산 -180, 진척도 -4. 10% 확률로 재잠적<br>C: 예산 -40, 진척도 -6. 이후 매일 10% 버그 이벤트(진척도 -3) |
| OF-25 | 제왕그룹발 뉴스 속보 | D-2 확정 발동 | 속보: '제왕그룹, 물류 자회사 구조조정 검토'. 아틀라스 담당 조직이 통째로 사라질 수 있다는 소문이 사무실을 훑는다. | A: 담당자 확인 전화<br>B: 동요 없이 작업 몰두<br>C: 차만수 인맥 가동 | A: 70% "저희 팀은 무사해요": 전원 멘탈 +5 / 30% "저도 오늘 알았어요": 전원 멘탈 -15, 최종 검수 담당자 교체(클라이언트 신뢰도 30으로 리셋)<br>B: 진척도 +4, 전원 멘탈 -5<br>C: 클라이언트 신뢰도 -5(뒷문 조회가 제왕 쪽에 알려진다), 결과 확정 공개 및 멘탈 변동 무효화 |
...
| 단계 | 발동 | 내용 | 결과 |
|---|---|---|---|
| 1/3 | 상기 표 참조 | 색상·버튼 "간단 수정" | 표 참조 |
| 2/3 | 1단계 익일 | "수정하신 김에 결재선 로직도 저희 방식으로." 수정이 수정을 부른다 | A: 수용 — 진척도 -8, 전원 멘탈 -10, 3단계 예약 / B: 구본철이 총대(반려 협상) — 구본철 멘탈 -15, 60% 체인 종료 |
...
#### OF-03 체인 — 반려의 늪 2단계
1단계 실패 시 익일 발동. "이번 사유: '전반적으로 다시'." A: 대표 직접 컨펌 후 제출 — 차만수 멘탈 -10, 클라이언트 신뢰도 +5, 통과 확정 / B: 반려 사유 목록을 붙여 정중히 역질의 — 50% 통과·클라이언트 신뢰도 +10 / 50% 클라이언트 신뢰도 -15.

#### OF-16 체인 — 한지우 이직 3부작
| 단계 | 발동 | 내용 | 결과 |
|---|---|---|---|
| 2/3 | D-2(또는 가속 조건) | 한지우 최종 합격 통보. "저… 드릴 말씀이 있는데요." | A: 붙잡기(진심) — 한지우 멘탈 40 이상이면 잔류: 멘탈 +20 / 미만이면 3단계 이탈 루트 / B: 축하해주기 — 한지우 멘탈 +30, 3단계 '유종의 미' 루트 / C: (1단계 B 해금) "루프 얘기 털어놓기" — 한지우 멘탈 +15, 잔류 확정, 야근 페이즈 동행 가능 해금 |
| 3/3 | D-1 | 이탈 루트: 한지우 자리가 비어 있다 — 진척도 -15, 전원 멘탈 -20. 후속 선택 — A: 공백을 있는 그대로 보고 — 추가 변동 없음 / B: **'테스트 결과 조작'(ED-07 연계)** — 한지우 몫의 미완 테스트를 통과한 것처럼 꾸며 서류상 진척도 차감을 -5로 은폐. D-0 아침 발각 판정(발각률 60%): 발각 시 ED-07 '제왕그룹 소송' 확정, 미발각 시 은폐 유지(엔딩 섹션 참조) / 유종의 미 루트: "마지막 날까지는 프로니까요" — 진척도 +10, 최종일 한지우 배치 효율 +20% | — |

#### OF-21 분기 — 옥상의 이직 상담(D-3 저녁 확정)

- **발동**: OF-21에서 A 또는 B 선택 시 D-3 저녁에 확정 발동. C 선택 시 미발생(상담 상대가 이미 떠날 결심을 굳히는 중이다).
- **상황**: 옥상 난간에 기댄 한지우가 먼저 입을 연다. "…그 헤드헌터, 저한테도 전화 왔었어요. 선배는 어떻게 하실 거예요?"
- **선택지와 결과**:
  - A: 비밀 지켜주기 — 한지우 유대도 +15, 한지우 멘탈 +10. **ED-13 '우리, 회사 차릴까요' 연계 플래그 ON**(발동 조건은 엔딩 섹션 ED-13 참조)
  - B: 구본철에게 보고 — 한지우 유대도 -20, OF-16 체인 즉시 2단계(발각 루트)
  - C: 서로의 고민만 나누고 내려온다 — 한지우 유대도 +5, 김신입 멘탈 +5
- **아이템 '포트폴리오 조각'(ED-14 연계)**: OF-21에서 B 선택 시 즉시 1개 획득, 이후 매일 결산마다 1개 자동 추가(최대 3개, 인벤토리 칸 비점유). D-2 밤까지 3개 수집 + 한지우 유대도 60 이상이면 **ED-14 '경쟁사에서 뵙겠습니다' 즉시 확정**(엔딩 섹션 참조).

#### OF-22 체인 — 세무조사 판정 2단계
1단계로부터 2일 후 발동. 무혐의: 대표 신뢰도 +5, 백은영 멘탈 +10 / 추징: 예산 -150, 차만수 멘탈 -30, 익일 OF-10 확정 발동.
...
| GH-08 | 수신인 불명 팩스 | D-6·D-3 고정 후보 | 어디에도 연결돼 있지 않은 팩스가 발신번호 '1997-0000'으로 문서를 뱉는다. 표지에 「아틀라스 구축 제안서(초안) — 작성: 정전무」라고 찍혀 있고, 종이는 아직 따뜻하다. | A: 12장을 끝까지 수신한다<br>B: 전원 코드를 뽑는다 | A: [1997 제안서 사본] 획득(GH-25 진엔딩 분기 재료), 수신 동안 멘탈 -10<br>B: 안전, 해당 런에서 재출현 없음 | A 1회 |
...
| GH-14 | 퇴근 행렬 | D-4 이후 / 야근 2인 이상 / 평균 멘탈 60 이하 | 밤 11시 정각, 사원증을 목에 건 그림자 수십이 벽을 따라 한 줄로 비상계단으로 사라진다. 행렬 맨 끝의 그림자가 멈춰 서서, 이쪽으로 목만 꺾어 자리 하나를 비워 보인다. | A: 행렬에 끼어 함께 걷는다<br>B: 야근 동료의 이름을 소리 내어 부른다 | A: 50% [1997년 사원증] 획득 후 이탈 성공 / 50% 상태이상 '동행'(3일 내 미해제 시 런 종료)<br>B: 행렬 소멸, 야근 전원 멘탈 -10, 이름 불린 동료 멘탈 +15 | A 생환 1회 |
...
| GH-16 | 미결(未決) | D-5 이후 / 대표 신뢰도 40 이하 | 얼굴 없는 양복 그림자가 결재판을 든 채 출입문을 등지고 서 있다. 결재란은 비어 있고, 그가 내미는 볼펜 끝에서 잉크 대신 검은 물이 뚝뚝 떨어진다. | A: 서명한다<br>B: "반려하겠습니다"<br>C: (백은영 야근 시) 실장에게 결재를 미룬다 | A: 대표 신뢰도 +10, 진척도 +5, 서명자 멘탈 -15, 3일 뒤 '이행 청구' 카드 확정(랜덤 자원 -10)<br>B: 담력 50% — [반려 도장](이벤트 카드 1장 무효화, 1회용) / 실패 — 문이 아침까지 잠김, 야근 전원 체력 -15<br>C: 무피해 격퇴(사내정치 9단은 유령 결재도 반려한다), 백은영 멘탈 -5 | B 성공 또는 C 1회 |
...
| GH-18 | 랙 사이의 것 | D-3 이후 / 진척도 60% 이상 | 서버실 랙과 랙 사이 어둠에서 팬 소음에 섞여 규칙적인 들숨 소리가 난다. LED 불빛이 일제히 깜빡이는 박자가, 그 숨소리와 정확히 일치한다. | A: 랙 사이로 손전등을 비춘다<br>B: 숨소리 박자에 맞춰 로그를 친다 | A: 담력 50% — [식은 하드디스크](진척도 +8 즉시) / 실패 — 서버 다운, 진척도 -12 + 구본철 멘탈 -20(복구 밤샘)<br>B: 동기화 — 그것이 밤새 빌드를 돌린다: 진척도 +10, 야근자 체력 -10, 20% [1997년 사원증] 드랍 | B 1회 |
...
| GH-20 | 월화수목금금금 | D-2 이후 / 해당 런 야근 누적 5회 이상 | 달력의 토·일 칸이 검게 문드러지더니, 일곱 요일이 척추처럼 이어진 거대한 것이 파티션 위로 몸을 세운다. 그것이 '금, 금, 금' 하고 우는 소리는 정확히 구본철 팀장의 한숨을 닮았다. | A: 남은 연차 서류를 던져 준다(예산 -20만 원)<br>B: 노동법전을 펼치고 "주 52시간!"을 외친다<br>C: 전원 즉시 퇴근한다 | A: 진정 — [달력 조각: 일요일](하루를 휴일 선언: 전원 멘탈 +25·체력 +25, D-day는 감소, 런당 1회)<br>B: 담력 35% — 격퇴 + [노동법전](야근 체력 소모 영구 -50%) / 실패 65% — 야근 전원 '빙의' 3일: 매일 자동 야근 강제(체력 -15/일, 휴식 불가)<br>C: 안전, 그날 야근 보상 전체 소실 | A 또는 B 성공 1회 |
| GH-21 | 가짜 차만수 | D-2 이후 / 대표 신뢰도 70 이상 또는 30 이하 | 퇴근했을 대표가 대표실 의자에서 등을 보인 채 골프 스윙 연습 중인데, 스윙이 관절을 무시하고 360도를 돈다. 돌아앉은 얼굴은 차만수가 맞지만, 눈코입의 배치가 사진을 잘못 이어 붙인 것처럼 미세하게 어긋나 있다. | A: 결재 서류를 내민다<br>B: 진짜 대표에게 전화해 확인한다<br>C: 골프 이야기를 꺼낸다 | A: 그것이 서명한다 — 예산 +50만 원, 대표 신뢰도 +20, 단 3일 뒤 진짜 차만수가 부정 — 대표 신뢰도 -40(단기 대출형 보상)<br>B: 담력 45% — 격퇴 + [대표의 만년필](대표 신뢰도 하락 페널티 영구 -50%) / 실패 55% — 김신입 '빙의' 3일(매일 밤 대표실에서 발견, 배치 불가)<br>C: 그것은 골프를 모른다 — 무피해 정체 간파, 멘탈 -10 | C 1회 또는 B 성공 |
...
| GH-23 | 갑(甲) | D-1 / 김신입 단독 야근 / 진척도 85% 이상 | 입사 때 서명한 '갑·을·병·정 계약서'가 책상 위에 스스로 펼쳐지고, '갑' 서명란의 글씨가 종이에서 일어나 붓글씨 획으로 이루어진 사람 형상으로 선다. 그것이 입 대신 낙관(落款)을 열고 말한다 — "을(乙)은, 납품하라." | A: 계약 이행을 서약한다<br>B: 계약서를 찢는다<br>C: [굵은소금 봉지]를 사용한다 | A: [갑의 낙관] — D-0 납품 판정에서 진척도를 +10%p 높여 간주(시스템 'D-0 납품 판정' 보정 항목, 검수 성공률 환산 +20%p). 대가로 '계약 연장' 플래그 고정 — 해당 런은 진엔딩(ED-16) 진입 불가, 납품 성공 시 일반 성공 엔딩(ED-01~03)으로만 정산<br>B: 담력 30% — 계약 파기: 진엔딩 분기 개방 + 멘탈 +30 / 실패 70% — 김신입 '빙의' 5일(사실상 런 종료급) + 진척도 -20<br>C: 갑 일시 후퇴, 선택을 다음 야근으로 유예(소금 소모) | 조우 1회(선택 무관) |
...
| ID | 괴이명 | 출현 조건 | 연출 묘사 | 선택지 A/B/(C) | 결과(리스크/리워드) | 도감 등록 |
|---|---|---|---|---|---|---|
| GH-24 | 체인 1/2 — 4층, 1997년 개발실 | GH-19에서 C 선택([1997년 사원증] 태그) / D-3 이후 | 문이 열린 4층은 CRT 모니터 수십 대가 켜진 1997년의 개발실이고, 모든 화면에서 같은 커서가 같은 박자로 깜빡인다. 라디오에서 IMF 구제금융 뉴스가 반복되는 가운데, 가장 안쪽 자리에서 한 남자가 등을 보인 채 27년째 같은 줄의 코드를 고치고 있다. | A: "선배님, 퇴근 안 하세요?"<br>B: 그의 모니터 속 버그를 대신 찾는다<br>C: 커피를 타서 책상에 올려 둔다(커피 -2) | 공통: 4층 체류로 체력 -15, 퇴장 시 1층 시계는 3분만 흘러 있음.<br>A: 정전무가 처음으로 돌아본다 — 멘탈 -20, 로어 해금: 「아틀라스」는 1997년 그가 설계하던 시스템의 이름과 같다<br>B: 디버깅 미니게임 60% 성공 — [빛바랜 플로피디스크](1997년산 · 아틀라스 설계 절반) / 실패 — 4층 추방, 3일 재진입 불가<br>C: "…골드라벨이네. 아직 파나." — [빛바랜 플로피디스크] 확정 획득 + GH-25 확정 예약 | 조우 1회(실루엣), B 성공 또는 C 시 전신 등록 |
| GH-25 | 체인 2/2 — 인수인계 | [빛바랜 플로피디스크] 소지 / D-2 이후 / 진척도 70% 이상 / 공포 게이지 70 미만(4.x.2 진엔딩 게이트) | 밤 9시 57분, 이번에는 4층이 아니라 '우리 사무실'에 정전무가 먼저 내려와 신입의 옆자리에 앉아 있다. 그가 27년 만에 처음으로 사원증을 목에서 벗으며 말한다 — "인수인계라는 걸, 한 번은 해 보고 싶었어." | A: 밤샘 인수인계를 받는다(야근 페이즈 전체 소모)<br>B: "선배님도 이제 퇴근하세요"<br>C: 함께 골드라벨 믹스커피를 마신다(커피 -2) | A: [백업 테이프] 획득 — [빛바랜 플로피디스크]+[백업 테이프] 조합 시 유물 [아틀라스 원본 설계도] 완성: 진척도 +25 즉시 + D-0 납품 판정 자동 성공(시스템 'D-0 납품 판정' 보정 항목). 대가: 김신입 체력 -30, 다음날 스크램블 -20초. 단 GH-23 '갑' 미격퇴 상태면 50% 확률로 갑이 난입, [백업 테이프]는 다음 루프로 이월<br>B: 성불 루트 — [정전무의 사원증](영구 장비: 전 괴이 우호화 + 모든 담력 판정 +20%p), [백업 테이프]는 소멸<br>C: 히든 — 루프를 넘어 A·B를 모두 경험한 회차라면 '동반 퇴근' 진엔딩(ED-16 「1997의 성불」) 플래그 점화: 납품 성공 시 4층이 소멸하고 정전무가 아침 햇빛 속에서 퇴근한다 | A·B·C 전 선택지 열람 시 도감 최종 페이지 「사번 0001 — 정전무」 완성 |
...
| [빛바랜 플로피디스크]·[백업 테이프] | GH-24 B·C / GH-25 A | 조합 시 [아틀라스 원본 설계도] — 진척도 +25 즉시, D-0 납품 판정 자동 성공 |
...
김신입이 서명한 '갑·을·병·정 계약서' 제7조에는 이렇게 적혀 있다. *"본 계약의 을(乙)은 실패한 시간을 반납할 수 있으며, 반납된 시간은 수당으로 정산한다."* 런이 실패하면 탕비실 자판기가 스스로 덜컹거리며 **골드라벨 믹스커피** 한 봉을 뱉어낸다. 이를 마시는 순간 화면이 커피색으로 물들며 D-7 아침 8시 59분으로 회귀한다. 회귀 직후 김신입의 사원증 뒷면에 숫자가 찍힌다 — 이것이 메타 재화 **야근수당(夜勤手當)** 이다. 아무도 주지 않았던 수당을, 루프가 대신 지급한다는 블랙코미디가 메타 진행의 서사적 뼈대다. 골드라벨 믹스커피의 자판기 재고는 **런당 충전**된다 — 루프 횟수에 제한은 없으며, '재고 = 루프 3회' 제한은 최고 난이도 '지옥의 크런치' 전용 규칙으로만 남는다(시스템 난이도 표 참조).
...
[사원증 각인 선택] → [전공 클래스 선택] → [저주받은 계약(선택)]
        ↓
[D-7 아침 스크램블] → 하루 루프 ×7 → [D-0 납품 판정(진척도 게이트+검수 굴림, 시스템 'D-0 납품 판정' 참조)]
        ↓                                    ↓
...
| PK-10 | 사내 정치학 개론 | 100 | 백은영 관련 이벤트에 [정치] 선택지 추가 | 백은영 멘탈 80 이상으로 D-4 도달 |
...
| ID | 이름 | 등급 | 효과 | 획득처 |
|---|---|---|---|---|
| RL-01 | 대표의 행운 골프공 | 희귀 | 대표 신뢰도 하락 1회 무효(소모) | 스크램블(대표실 고가치 스폰 슬롯 — 평시 잠김, 매일 아침 25% 개방·차만수 유대도 60 이상 시 50%, 시스템 2.1 참조) |
| RL-02 | 기계식 키보드 | 일반 | 개발 슬롯 진척도 +1%p | 스크램블(한지우 자리) |
| RL-03 | 소금 부적 | 일반 | 괴이 조우 1회 자동 퇴치(소모) | 스크램블(창고 구역 부적 슬롯) |
| RL-04 | 탕비실 마스터키 | 희귀 | 매일 커피 +1, 간식 +1 자동 수급 | 스크램블(백은영 서랍, D-4 이후) |
...
| RL-10 | 구 팀장의 라이터 | 일반 | 옥상 이벤트 해금, 사용 시 구본철 멘탈 +5 | 스크램블(구본철 책상) |
| RL-11 | 모니터 뒷면 부적 스티커 | 일반 | 김신입 멘탈 감소량 -10% | OF-13(라이선스 만료) — 퇴사자 메일함 정리 중 발견 |
...
| RL-20 | 갑질 방지 이어플러그 | 일반 | 클라이언트 응대 슬롯의 멘탈 감소 0 | 스크램블(한지우 파티션) |
...
##### 5.2 퇴사자 명패 (전 8개)
이면 오피스 각 구역에 1개씩 숨겨진 전(前) 직원들의 명패. 획득 시 그 퇴사자의 마지막 밤 사연 로그(1997~2024년, IMF·닷컴버블·코로나 야근 등 시대별 에피소드)가 열리고, **명패 1개당 전원 멘탈 최대치 +2 영구 증가**. 8개를 모두 모으면 '퇴사자의 그림자'가 적대 괴이 목록에서 빠지고 이면 오피스의 길잡이 NPC가 된다.

##### 5.3 엔딩 갤러리 (전 17종, ED-01~17)
최초 달성 시 엔딩별 야근수당 보상이 정산 공식과 별도로 지급된다(수치는 엔딩 섹션 보상란이 정본). 대표 엔딩 예시: ED-01 턱걸이 납품(노멀 성공) / ED-02 완벽 납품 / ED-03 전설의 납품(진척도 120% 상한 도달) / ED-04 부도(기본 실패) / ED-05 전원 번아웃 / ED-06 대표 잠적 / ED-13 우리, 회사 차릴까요(한지우 인간관계 루트) / ED-14 경쟁사에서 뵙겠습니다(이직 루트) / ED-10 이면 오피스 상주 인원(호러 배드) / ED-11 4층의 주민(히든) / ED-16 1997의 성불(진 — 정전무 서사 완결). 갤러리에서는 엔딩 일러스트·달성 조건·달성 당시 스탯이 열람 가능하다.
...
| 축 | 랜덤화 내용 | 목적 |
|---|---|---|
| 스크램블 배치 | 아이템 위치가 매 런 셔플, 대표실(평시 잠김)은 매일 아침 개방 굴림(25%, 차만수 유대도 60 이상 시 50% — 시스템 2.1 참조) | 아침 60초의 동선 최적화가 매번 달라짐 |
| 이벤트 덱 | 현실 이벤트 25종 + 이면 괴이 25종 + 인물 전용 훅 + 체인 후속 카드로 구성된 풀에서 런당 21장 추첨, 일차별 테마 가중치(초반 업무·중반 정치·후반 괴이) | 같은 클래스라도 대응 빌드가 달라짐 |
| 주간 변수 | 런당 1개 부여: '클라이언트 요구 변경'(진척도 -15%로 시작), '장마'(전원 멘탈 -10% 지속), '골프 시즌'(대표 부재로 대표 신뢰도 이벤트 없음) 등 6종 | 런 전체의 전략 방향 결정 |
...
- 엔딩은 **즉시형**(런 중 조건 충족 프레임에 곧바로 확정)과 **결산형**(D-0 아침 납품 판정에서 분기 — 진척도 게이트·검수 판정 절차는 시스템 섹션 'D-0 납품 판정'이 정본)으로 나뉜다.
...
|---|---|---|---|---|---|
| ED-01 | 턱걸이 납품 | D-0 납품 판정(검수) 성공 + 진척도 100~114%, ED-02·03 조건 미달 | 노멀 | 서버를 여는 순간 에러 알림이 폭주하지만 검수는 기적적으로 통과한다. 차 대표가 "다음 프로젝트도 이 멤버로!"를 외치는 동안 전원이 책상 밑에서 이직 앱을 켠다. | 야근수당 100, 아이템 '테이프 감은 키보드' 언락 |
| ED-02 | 완벽 납품 | D-0 납품 판정 성공 + 진척도 ≥115%, 4인 전원 멘탈 ≥40, 미해결 버그 이벤트 0건 | 굿 | 제왕그룹 담당자가 처음으로 "수고하셨습니다"를 존댓말로 말한다. ㈜버티고는 13년 만에 첫 흑자 분기를 맞고, 김신입의 자리에 처음으로 명패가 놓인다. | 야근수당 250, 시작 인벤토리 슬롯 +1 |
| ED-03 | 전설의 납품 | D-0 납품 판정 성공 + 진척도 120%(상한) 도달, 전원 멘탈 ≥70, 대표 신뢰도 ≥80, 런 전체 야근 2회 이하 | 굿(최상위) | 야근 없이 마감한 SI라는 소문이 업계에 괴담처럼 퍼지고, 제왕그룹이 유지보수 3년 계약을 선결제한다. 차 대표는 골프채를 팔아 전 직원 성과급을 쏜다. | 야근수당 500, 스킨 '황금 사원증', 하드모드 'D-5' 언락 |
| ED-04 | 부도 | D-0 납품 판정 실패 — 진척도 100% 미만(게이트 불통과) 또는 검수 판정 실패(시스템 'D-0 납품 판정' 참조) | 배드 | 납품 실패 위약금 통지서가 팩스로 도착하고, 사무실 형광등이 하나씩 꺼진다. 12명의 책상은 중고 가구 업자의 트럭에 실려 나간다. | 야근수당 30 |
| ED-05 | 전원 번아웃 | 동시점에 동료(김신입 제외) 3인 이상 멘탈 0 이탈(붕괴 처리는 시스템 1.1 '멘탈 0 / 체력 0 붕괴 처리 표' 참조. 김신입 본인의 멘탈 0은 엔딩이 아니라 루프 강제 리셋) | 배드 | 어느 아침, 출근한 사람이 김신입뿐이다. 단체 채팅방에는 "죄송합니다"만 세 개 떠 있고, 아틀라스는 영원히 미완성으로 남는다. | 야근수당 40, 아이템 '탕비실 안마의자' 언락 |
| ED-06 | 대표 잠적 | 대표 신뢰도 0 도달 | 배드 | 차 대표가 법인 도장과 함께 사라지고 대표실에는 골프백만 남는다. 백 실장이 "내 이럴 줄 알았다"며 서랍에서 이력서를 꺼낸다. | 야근수당 40 |
| ED-07 | 제왕그룹 소송 | OF-16 체인 3단계(이탈 루트) 선택지 B '테스트 결과 조작' 선택 후 D-0 아침 발각 판정(발각률 60%)에서 발각, 또는 진척도 허위 보고 누적 3회 | 배드 | 내용증명이 등기로 도착하고 뉴스 자막에 회사 이름이 이니셜로 나온다. 김신입은 참고인 조사에서 "시키는 대로 했을 뿐"이라고 진술한다. | 야근수당 50, 이벤트 카드 '내부고발' 언락 |
...
| ED-13 | 우리, 회사 차릴까요 | 한지우 유대도 ≥80 + 납품 성공(ED-01~03 조건 충족) + OF-21 분기 카드 '옥상의 이직 상담'(D-3 저녁 확정 발동)에서 A '비밀 지켜주기' 선택 | 굿(인간관계) | 납품 다음 날, 한지우와 김신입은 나란히 사직서를 내고 2인 스타트업을 차린다. 첫 사무실 탕비실에는 믹스커피를 절대 두지 않기로 한다. | 야근수당 300, 뉴게임+ 시작 동료 '한지우' 언락 |
| ED-14 | 경쟁사에서 뵙겠습니다 | OF-21 '경쟁사 스카우트 전화'(D-5~D-4)에서 B '제안 수락' 선택 + 한지우 유대도 ≥60 + D-2 밤까지 아이템 '포트폴리오 조각'(OF-21 B에서 획득 개시, 매일 결산 +1·최대 3개) 3개 수집 → D-2 밤 즉시 확정 | 노멀(인간관계) | 납품 이틀 전, 김신입은 연봉 1.8배의 오퍼를 쥐고 회사를 떠난다. 엘리베이터가 닫히기 직전 보인 구 팀장의 표정은 두고두고 꿈에 나온다. | 야근수당 150, 퍽 '경력직 보정(시작 진척도 +5%)' 언락 |
...
| ED-16 | 1997의 성불 | GH-19(C 선택)→GH-24→GH-25 체인 완료 + GH-25 선택지 C + D-0 납품 성공(하단 상세 시나리오의 전 조건 충족) | 진 | 27년 만에 정전무가 퇴근하고, 그의 이름이 아틀라스 커밋 로그 첫 줄에 새겨진다. D-0 아침, 자판기는 처음으로 커피를 내놓지 않는다 — 루프는 끝났다. | 야근수당 1000, 진 오프닝 + 뉴게임+ 모드 '정전무의 유산' 언락 |
...
| 진척도 | GH-25 발동 시점 70% 이상 + D-0 납품 판정(시스템 'D-0 납품 판정') 성공 |
...
- 연출: 문이 열리면 1997년 12월의 개발실. CRT 모니터 수십 대에서 같은 커서가 같은 박자로 깜빡이고, 라디오에서 IMF 구제금융 뉴스가 반복된다. 담배 연기와 "IMF 극복" 현수막 아래, 가장 안쪽 자리에서 정전무가 등을 보인 채 27년째 같은 줄의 코드를 고치고 있다.
- 핵심 획득: **[빛바랜 플로피디스크]**(1997년산 · 아틀라스 설계 절반) — C '커피를 타서 책상에 올려 둔다'(커피 -2)로 확정 획득 + GH-25 확정 예약("…골드라벨이네. 아직 파나."), 또는 B(그의 모니터 속 버그를 대신 찾는 디버깅 미니게임, 성공률 60%)로 획득. B 실패 시 4층 추방·3일 재진입 불가.
- 공통 대가: 4층 체류로 체력 -15. 퇴장하면 1층의 시계는 3분만 흘러 있다.

**체인 2/2 「인수인계」 — GH-25**

- 발동: [빛바랜 플로피디스크] 소지 / D-2 이후 / 진척도 70% 이상 / 공포 게이지 70 미만.
- 연출: 밤 9시 57분, 이번에는 4층이 아니라 '우리 사무실'에 정전무가 먼저 내려와 신입의 옆자리에 앉아 있다. 그가 27년 만에 처음으로 사원증을 목에서 벗으며 말한다 — "인수인계라는 걸, 한 번은 해 보고 싶었어."
- 세 갈래 선택지(결과 수치는 판타지 섹션 GH-25 결과란이 정본):
  - **A(성과 루트)**: 밤샘 인수인계(야근 페이즈 전체 소모) → **[백업 테이프]** 획득. [빛바랜 플로피디스크]+[백업 테이프] 조합 시 유물 [아틀라스 원본 설계도] 완성 — 진척도 +25 즉시 + D-0 납품 판정 자동 성공. 대가는 김신입 체력 -30·다음날 스크램블 -20초이며, 성불은 이루어지지 않는다.
...
**최종 연출 「마지막 커밋 — 동반 퇴근」 (플래그 점화 + D-0 납품 성공 시)**

장면 1: 검수 통과가 확정되는 순간 사무실 전체가 잠시 이면 오피스로 전환된다(플래그 점화 회차 한정의 유일한 강제 전환). 복사기 요괴도, 퇴근 행렬의 그림자도 모두 벽 쪽으로 물러나 길을 비킨다.

장면 2: 정전무가 처음으로 얼굴을 보인다. 그는 27년째 함수 하나를 닫지 못하고 있다 — 그의 무한 루프가 이 건물 타임루프의 근원이며, 골드라벨 믹스커피는 그가 그날 밤 마시던 커피였다는 반전이 공개된다. 아틀라스 커밋 로그 첫 줄에 그의 이름이 새겨지고, 형광등이 하나씩 꺼진 뒤 새벽 첫차 소리와 함께 정전무가 처음으로 '1층' 버튼을 누른다. 4층은 소멸하고, 그가 아침 햇빛 속에서 퇴근한다 — **ED-16 확정**.

**루트별 변형·특례**

| 루트 | 내용 | 결과 |
|---|---|---|
| GH-25 A(성과 루트)로 납품 | [아틀라스 원본 설계도]로 검수 자동 성공, 성불 없음 | 통상 결산형 엔딩(ED-01~03·13)으로 분기. ED-02 조건 충족 시 뒤틀린 변형 **'B사이드'**(커밋 로그에서 내 이름이 날마다 흐려진다) 에필로그로 대체되고, 다음 회차 이면 오피스 공포 게이지 증가량 +25% |
| GH-25 B(성불 루트) | 정전무는 떠나고 [백업 테이프]는 소멸 | 진엔딩 아님. C 개방을 위한 '경험 플래그'가 계정에 적립된다 |
| GH-25 C + D-0 납품 실패 | 플래그는 점화됐으나 회사가 무너진다 | **'ED-04 변형: 성불 유예'** — 결산은 부도이나 에필로그 텍스트가 교체되고, 플래그는 계정에 영구 저장되어 다음 회차 시작 야근수당 +100 |
...
4. **결산형(D-0) 판정 순서:** 납품 판정(시스템 'D-0 납품 판정') 성공 시 ED-16 진 > ED-13 > ED-03 > ED-02 > ED-01 순으로 검사하고, 판정 실패 시 ED-04. 상위 판정 성립 시 하위는 검사하지 않는다.
...
| 11 | ED-07 발각 판정과 D-0 결산이 같은 날 | ED-07 | 발각 판정은 납품 판정(시스템 'D-0 납품 판정')보다 먼저, D-0 아침 09:00에 선행 처리 |
| 12 | GH-25 C 플래그 점화 + D-0 납품 실패 | ED-04 변형(성불 유예) | 결산 규칙상 부도이나, 에필로그 텍스트 교체 + 영구 야근수당 보너스로 보상 |

**구현 메모:** 엔딩 판정기는 `즉시형 큐(우선순위 정렬) → D-0 아침 발각 판정 → 납품 판정(진척도 게이트+검수, 시스템 'D-0 납품 판정') → 결산형 순차 검사`의 4패스로 구현한다. '스쳐간 엔딩' 표시는 도감 수집률에 포함되지 않으며, 힌트로만 기능한다.
...
- **개인 산출** = WP × 적성 배수 × C, C = 0.5 + 0.005 × min(멘탈, 체력) (3.1). 한지우 패시브 '에이스의 대가'는 개발 산출의 +30%를 별도 가산하고, 매일 자동 멘탈 -5를 적용(캐릭터 4.2).
- **일일 진척도** = Σ개발 + 0.5×Σ문서 + 야근 보너스(참여 인원 산출 합 × 0.6) + 시너지·아이템 보정 − 이벤트 페널티(3.5). 구본철 야근 참여 시 '야근의 화신' ×1.5 및 익일 체력 -10 반영.
- **시너지**: 구본철+한지우 개발 페어는 **+1.5%p 플랫 가산**(3.4 표 준거).
...
- **판정**: D-0 게이트 100% 미만 즉시 ED-04, 통과 시 검수 성공률 = 50 + (진척−100)×2 + (클신−50)×0.5 (5절).
...
- **모델 단순화**: 체력 0 병가는 '이후 배치 불가'로 근사(복귀 보정 오차 ±4%p 이내), 확률 판정(검수·담력)은 성공률 수치로 표기하고 굴림하지 않는다.

#### 1. 표준 7일 시뮬레이션 3종

##### 1.1 ⓐ 무지성 플레이 — 야근 없음, 고정 배치(개발 한+구 / 문서 백+차), 스크램블 스킵, 선택지 전부 A

| 일차 | 진척도 누계 | 커피/간식/예산(만) | 한지우 m/h | 구본철 m/h | 김신입 m/h | 공포 | 발생 가정 이벤트 |
|---|---|---|---|---|---|---|---|
| D-7 | +26.7 → 26.7 | 8/0/300 | 17/37 | 27/27 | 43/55 | 0 | 간식 0 → **허기 디버프**, OF-20 B(김 멘탈 -10) |
| D-6 | +20.9 → 47.6 | 8/1/300 | 4/29 | 9/**0** | 33/52 | 0 | OF-03 A(구 -10/-20, 진척 -3), 콜 응대. 구본철 체력 0 → 병가 |
| D-5 | +7.9 → 55.5 | 8/0/300 | **0 이탈** | **0 이탈** | 20/34 | 0 | OF-01 강제 A(진척 -5, 한 -10)+허기 — 한지우는 휴식을 줘도 이벤트가 상쇄 |
| D-4 | −0.6 → 54.9 | 11/3/300 | — | — | 2/26 | 0 | OF-01 2/3 A(진척 -8, 전원 -10) |
| D-3 | +5.0 → 59.9 | 11/0/300 | — | — | 12/38 | 0 | OF-17 B(+5)·OF-23 A·평가판 중단(-4), 김 휴식, 허기 |
| D-2 | +6.4 → 66.3 | 11/3/300 | — | — | **0** | 0 | OF-25 B + OF-01 3/3 A(전원 -20) → 차·백 이탈, **동료 3인 이상 이탈 = ED-05 조기 확정** |
| D-1 | (−4.0) → 51.5 | 11/3/300 | — | — | — | 0 | (계속 가정 시) 통합테스트: 버그 21.7 → -10.8 |
| **D-0** | **51.5%** | — | — | — | — | — | **게이트 불통과 → ED-04** (실제로는 D-2 ED-05 선행) |

**검산**: D-7 개발 = 한 10×1.5×0.675=10.1(+패시브 3.0) + 구 8×1.2×0.7=6.7 + 시너지 1.5 = 21.3 / 문서 = (백 8.7+차 3.2)×0.9(차백 문서 역시너지)×0.5 = 5.4 → 26.7. 버그 21.3×0.4=8.5. 간식 4−배치 4인=0 → 허기.

##### 1.2 ⓑ 균형 플레이 — 야근 2~3회(D-4·D-2·D-1), 김+한 페어(멘탈 소모 -50%)·백+구 문서(+15%)·커피 셔틀 격일, 휴식 로테이션(한 D-5, 김 D-3·D-2, 구 D-2)

| 일차 | 진척도 누계 | 커피/간식/예산(만) | 한지우 m/h | 구본철 m/h | 김신입 m/h | 공포 | 발생 가정 이벤트 |
|---|---|---|---|---|---|---|---|
| D-7 | +21.8 → 21.8 | 11/6/300 | 26/47 | 34/41 | 52/57 | 0 | 랜덤 기대 손실 -4, 픽업 IT-01·04 |
| D-6 | +16.5 → 38.3 | 11/7/250 | 27/44 | 28/36 | 44/49 | 0 | 콜: 차+백 응대(+20%p→85%), OF-03 C 외주(-50만) |
| D-5 | +7.6 → 45.9 | 11/10/250 | 37/56 | 24/28 | 36/41 | 0 | OF-13 C(-6), OF-01 B(클신 -5, 체인 종료), 한 휴식 |
| D-4 | +23.9 → 69.8 | 9/12/250 | 34/53 | 6/**0** | 20/24 | 0 | 콜 응대, **야근(김+구) +7.0**, 셔틀→한 |
| D-3 | +20.9 → 90.7 | 14/10/250 | 21/55 | 10/10 | 35/46 | 0 | OF-17 C(구 +10)·OF-23 A(전원 체력 +10), 김 휴식 |
| D-2 | +29.3 → **120.0** | 12/14/249 | 13/52 | **0 이탈** | 35/46 | 5 | OF-25 B, **야근(김+구) +6.7** — 구본철 마지막 야근 후 멘탈 0 |
| D-1 | +12.9 → 104.1 | 11/23/248 | **0 이탈** | — | 19/29 | 5 | 야근(김 단독) +1.9, **통합테스트 버그 31.8 → -15.9** |
| **D-0** | **104.1%** | 클신 60 | — | — | 생존 | 5 | 게이트 통과, **검수 63.2% → 성공 시 ED-01 '턱걸이 납품'** |

**검산**: D-4 = 개발(한 10.1+3.0 + 구 5.6 + 1.5) + QA 김(버그 -3.2) + 야근 보너스(김 3.1 + 구 4.6×1.5)×0.6=7.0 − 이벤트 -4 = 23.9. 검수 = 50+(104.1−100)×2+(60−50)×0.5 = 63.2%.
...
목표 대비 판정: **ⓑ만 목표 달성**(104.1%·검수 63% = 턱걸이). **ⓐ는 목표(70~85% 좌초)보다 깊게 붕괴**(51.5%, D-2 ED-05 선행) — 이벤트의 전원 멘탈 페널티가 낮은 초기 멘탈(구 40·한 35)과 중첩돼 D-5에 연쇄 이탈이 시작된다. **ⓒ는 '도박'이 아니라 성립 불가** — 갈아넣기 -25는 한지우 초기 멘탈 35에서 하루 소모 합 -48을 만들어 첫날 이탈시킨다.

| 파라미터 | 현재값 | 권고값 | 근거 |
|---|---|---|---|
| 한지우 '한 번만 갈아넣기' 멘탈 대가 | -25 | **-15** | 최대 50·초기 35 대비 -25는 사용 = 이탈. ⓒ 아키타입의 존재 조건 |
| 야근 멘탈 소모 | -10 | **-7** | 매일 야근이 3일 내 전원 붕괴. -7 + 컵라면(+5)로 '관리하면 버티는' 도박 성립 |
| 런 시작 멘탈(자원표) | 구 40 / 한 35 | **구 45 / 한 40** | ⓐ 좌초 시점을 D-5 → D-3~D-2로 지연, 좌초 대역을 70~85로 이동 |
...
**권고안 적용 재검증**(동일 스케줄 재시뮬): ⓐ' 피크 89.5 → 통테 후 **72.6% 좌초**(목표 밴드 진입) / ⓑ' **102.9%·검수 60.8%**(턱걸이 유지) / ⓒ' **D-3에 상한 120% 도달**, 통테 후 107.2%·검수 71.9% — 단 구본철 이탈, 김신입 번아웃 2회에 D-1 체력 0 직전(IT-07 홍삼 미확보 시 그 자리에서 ED-09), 공포 게이지 40(담력 판정 -10%p, ED-10 사정권). 진척도는 넘치지만 **ED-09·ED-10·멘탈 리셋 3중 리스크를 안은 도박**으로 목표 정체성이 성립한다.

#### 3. 난이도별 목표 승률

| 난이도 | 목표 승률 | 근거(권고안 적용 기준) |
|---|---|---|
| 이지 | 75% | 게이트 90%(일 12.9%p) + 소모 ×0.8 → ⓑ급 플레이의 완주율 ~90%, 검수 기대 +28%p 구간 ≈ 0.9×0.83 |
| 노멀 | 45% | ⓑ 검수 63% × 완주 도달률 ~70%(멘탈 사고·체인 분산) ≈ 0.44 — 시뮬과 정합 |
| 하드 | 25% | 게이트 110% + 소모 ×1.25 + 콜 4회 → 야근 3~4회가 강제되고 번아웃 관리가 필수. 통과율 0.6×검수 0.55×완주 0.75 |
...
| 예산 | 300 + 이벤트·스킬 수입(회장님 골프 +80×최대 3회, D-4 한도 +200) | 이벤트 기대 -30~-50/일(-210~-350) + 구매 ~-15 | 균형 플레이 잔액 기대 40~120만 → **OF-10(100만 미만 확정) 발동률 ~60%** — '법카 만능' 견제 압박이 수치로 성립 |
...
- **D-6 「낮의 의무, 밤의 대가」**: 첫 클라이언트 콜(공석 시 클신 -10)과 확정 이벤트 OF-03이 응대·문서 슬롯의 존재 이유를 가르치고, 구본철(체력 최저 45)이 최초의 체력 0 후보가 되며 소모 관리가 등장한다. 야근을 선택하면 밤 9시 57분, 기본 35%+보정의 첫 이면 조우 — 낮 자원 게임 위에 밤 리스크 게임이 겹쳐진다.
- **D-5 「에이스는 관리 대상이다」**: 확정 OF-13(예산 -90 / 매일 15% 중단 / 진척 -6의 3지선다)이 '자원 교환' 사고를 요구하고, OF-01 강제 마감이 체인 이벤트의 개념을 각인한다. 그리고 한지우 — 초기 멘탈 35에서 개발 -8·패시브 -5로 이틀 만에 임계(9)에 닿는다. 휴식·페어링 없이 D-5를 넘기지 못하는 이 붕괴가, 이 게임의 핵심 질문 "누구를 갈아 넣을 것인가"를 첫 루프 안에 반드시 체험시킨다.
...
1. **튜토리얼은 곧 신입 연수다.** 별도의 튜토리얼 모드를 두지 않는다. 루프 1회차의 D-7 하루 전체가 스크립트드 온보딩이며, 모든 UI 안내는 세계관 내 화자(백은영 실장의 지시, 한지우 대리의 귓속말, 사내 메신저 '버티톡' 알림)가 수행한다. 시스템 보이스의 무미건조한 팝업은 금지. 플레이어가 "게임을 배웠다"가 아니라 "첫 출근을 버텼다"고 느끼게 한다.
...
| S-02 | 컷씬 종료 | 스크램블 맵(타이머 OFF) | 이동(WASD/가상 스틱). "내 자리로 가세요" 화살표+바닥 발자국 라인 | 유도 | 20초 무입력 시 버티톡 알림으로 조작키 재안내, 40초 무입력 시 반투명 손가락 데모 재생 |
...
| S-05 | 오버레이 닫기 | 스크램블(연습 모드) | **45초 연습 스크램블.** 맵에 아이템 6종만 스폰(커피믹스·초코파이·노트북 충전기·법인카드·구급상자·USB). 필수 3종(커피믹스·충전기·법인카드) 금색 테두리 표시 | 자유 | 시간 만료 시 백은영: "신입이 첫날부터 완벽하면 그게 더 무섭지." 미획득 필수품은 한지우가 책상에 놓아줌(자동 보정 지급). 획득 0개면 1회 한정 재도전 제안(거절 가능) |
| S-06 | 연습 종료 | 업무 페이즈 | 화면 구조 개관(인물 카드/슬롯 5종). 첫 지시: 한지우를 '개발' 슬롯에 드래그 | 강제(해당 드래그만 허용) | 잘못된 슬롯 드롭 시 카드가 제자리로 스냅백 + 목표 슬롯 점멸. 30초 경과 시 자동 데모 |
| S-07 | 한지우 배치 완료 | 업무 페이즈 | 나머지 3인(차만수 제외) 자유 배치. 슬롯 툴팁(진척도 기여치/멘탈 소모) 열람 유도 | 유도 | 미배치 인물 존재 시 '하루 시작' 버튼 비활성 + 미배치 카드 흔들림. 배치 강요는 하지 않고 '휴식' 슬롯도 정답 처리 |
...
| S-10 | 이벤트 종료 | 야근 선택 팝업 | 야근의 개념. **첫날은 스토리상 강제 야근**(구본철: "신입은 원래 남는 거야"). '퇴근' 버튼은 보이되 비활성+자물쇠 아이콘으로 '선택지가 원래 있음'을 예고 | 강제 | 실패 없음 |
...
| S-14 | D-6 아침 시작 | 스크램블(첫 실전) | **진짜 60초.** 전체 아이템 풀 스폰. 보정: 이번 1회 한정, 미획득 필수품 1개를 결산 직전 자동 지급(문구: "한 대리가 몰래 챙겨뒀다") | 자유 | 보정 1회 소진 후 D-5부터 무보정. 필수품 0개 획득 시에만 재도전 1회 제안 |
...
| 조항 | 규칙 | 적용 예 |
|---|---|---|
| 제1조 데드팬 원칙 | 인물은 절대 웃기려 하지 않는다. 상황이 웃기고, 인물은 끝까지 진지하다. 개그 대사·말장난으로 웃음을 만들지 말고, 진지한 대사와 부조리한 상황의 낙차로 만든다. | 차만수: "우리 회사의 유일한 자산은 사람이야. 그러니까 사람을 갈아야지." |
| 제2조 단문 우선 | 지문·대사 모두 1문장 40자 이내. 쉼표 2개 이상 금지. 긴 설명이 필요하면 문장을 쪼갠다. 리듬이 곧 유머다. | "회의가 끝났다. 회의 일정을 잡는 회의였다." |
| 제3조 구체성의 고통 | 고통은 추상어("힘들다")가 아니라 고유명사·수치·파일명으로 쓴다. | 『아틀라스_최종_진짜최종_수정3_v7(final).xlsx』 / "재석 12, 멘탈 3." |
| 제4조 조준 방향 | 풍자의 총구는 항상 시스템(관행·구조·마감)을 향한다. 개인의 외모·출신·가정사를 조롱의 소재로 쓰지 않는다. 차만수조차 '나쁜 사람'이 아니라 '나쁜 구조의 산물'로 그린다. | O: 결재 라인 5단계 풍자 / X: 인물 외모 개그 |
...
| 제5조 단어 오염 | 낮에 쓴 일상 단어를 밤에 다른 의미로 재사용한다. 같은 단어가 두 번째 등장할 때 공포가 된다. | 낮: "야근자 명단 올려" → 밤: 명단 12번째 줄에 퇴사자 이름 |
...
| 실존 기업·인물 연상 금지 | 실존 브랜드·기업명·로고 패러디 금지. 가상 브랜드('골드라벨', '제왕그룹', '아틀라스')만 사용. 실존 기업과 2음절 이상 겹치는 신규 명칭은 리드 승인 필수. |
...
| 이모티콘 금지 | UI·시스템 텍스트에 이모지 사용 금지. 단, 차만수의 단톡 메시지 안에서만 예외적으로 허용("^^7") — 캐릭터성 연출 목적. |

#### 1.4 인물별 말투 매트릭스

행 = 화자, 열 = 청자. 셀 = 종결 어미 체계 + 특징.

| 화자 \ 청자 | 차만수 | 백은영 | 구본철 | 한지우 | 김신입 |
|---|---|---|---|---|---|
| **차만수** | — | 반말+의존 ("백 실장이 알아서 해봐") | 반말+골프 비유 ("구 팀장, 어드레스가 틀렸어") | 반말+이름 자주 틀림 ("한... 한 대리!") | 반말+호칭 생략 ("어이, 신입") |
| **백은영** | 격식 존대 ("대표님, 그건 어렵습니다") | — | 건조한 해요체+평서 명령 ("금요일까지 됩니다. 되게 하세요") | 건조한 해요체 | 건조한 해요체 ("김신입 씨") |
| **구본철** | 다급한 존대 ("대표님, 저희가 진짜...") | 다급한 존대 | — | 반말+부탁조 ("지우야, 미안한데...") | 반말+부탁조 ("신입, 미안한데...") |
| **한지우** | 최소한의 존대(단답) | 최소한의 존대(단답) | 존대+한숨 선행 | — | 해요체+유일하게 문장이 길어짐 (신입에게만 마음을 엶) |
...
- 정전무는 1997년식 어휘를 쓴다: "디스켓", "전산실", "야마가 안 잡히네". 2020년대 신조어 사용 절대 금지.
...
**A-1. 2호선 지하철 내부 / 07:41 / 횡스크롤 일러스트**
...
김신입 (독백) 입사 3개월 차. 아직도 모르겠다. 내 사수가 누군지. 내 일이 뭔지. 그리고 입사 서류가 왜 네 장이었는지.

【화면】 휴대폰 클로즈업. 단톡방 「㈜버티고 전직원(12)」

- 06:50 차만수 대표: 오늘부터 D-7 총력전!!! 다들 아자아자 ^^7
- 03:12 구본철 팀장: 빌드 올렸습니다. 확인 부탁드립니다.
- 03:14 구본철 팀장: 죄송합니다 방금 거 아닙니다. 다시 올렸습니다.
- (한지우 대리 — 읽음 표시만. 3개월째 발언 없음)

김신입 (독백) 우리 회사 조직도는 이 단톡방이 전부다.

**A-2. ㈜버티고 사무실 입구 / 08:29**

【SFX】 도어락 4자리 비프음
【화면】 탑다운 뷰로 전환. 사무실 전경 최초 공개(책상 구역/탕비실/서버실 문/회의실/창고).

백은영, 입구 정면에 서 있다. 태블릿을 안고. 김신입이 문을 열자마자.

**백은영** 8시 29분. 지각은 아니네요.

**김신입** 아, 안녕하세— 

**백은영** 인사는 넣어두세요. 오늘부터 그럴 시간 없습니다.

【화면】 책상 구역. 구본철이 키보드에 이마를 대고 엎드려 있다. 모니터에는 빌드 진행바 97%.

**구본철** (고개를 들지 않고) ...신입 왔냐. 미안한데 탕비실에서 커피 좀. 아니다. 두 개.

【화면】 창가 자리. 한지우, 이어폰. 모니터 두 대 중 한 대에 이력서 창이 0.5초 보였다가 알트탭. (플레이어만 목격 가능한 프레임 — 떡밥)

**한지우** (이어폰 한쪽만 빼고) ...탕비실 커피, 왼쪽 통은 디카페인이에요. 팀장님 거는 오른쪽에서 타요. 왼쪽 타 가면 화내요.

**김신입** (독백) 3개월 만에 들은 가장 긴 문장이었다.

【SFX】 문이 벽에 부딪히는 소리
【화면】 차만수 등장. 골프백을 어깨에 멘 채.

**차만수** 자자, 주목! 어제 제왕그룹 상무님이랑 라운딩을 했어요, 내가.

**백은영** (조용히) 스크린 골프요.

**차만수** 라운딩을 했는데! 상무님이 그러시더라고. "차 대표, 아틀라스 일주일 남았지?" 이게 무슨 뜻이겠어?

**구본철** (겨우 상체를 세우며) ...일주일 남았다는 뜻 아닙니까.

**차만수** 기대하신다는 뜻이야! 우리를! 이 버티고를! (골프백을 내려놓으며) 납품 무조건 성공한다. 실패하면? 

정적. 전원 타자 멈춤.

**차만수** (씩 웃으며) 실패 안 하면 되지.

**백은영** (태블릿을 켜며) 정리하죠. 납품까지 7일. 현재 진척도 34%. 회의는 60초 뒤 시작합니다. 각자 필요한 거 챙겨서 회의실로.
...
【화면】 사무실에 김신입과 구본철 둘뿐. 창밖은 검다.

**구본철** (기지개) 신입, 야근 처음이지? 요령 알려줄까. 9시 넘으면 시간이 안 가. 보지 마, 시계.

**김신입** 팀장님은... 매일 이 시간까지 계세요?

**구본철** (모니터를 보며) 우리 애가 그러더라. 아빠는 회사에서 사는 사람이라고. (사이) 웃기지? 웃으라고 한 얘기야. (웃지 않는다) 편의점 갔다 온다. 컵라면 물 올려놔.
...
**구본철** (봉지를 들고 들어오며) 어? 복사기 고쳤네. 그거 3주 전부터 말썽이었는데. 신입, 능력 있다?
...
**김신입** 누, 누구세요. 여기 직원...이세요?

**정전무** (자판을 치며. 소리만 나고 화면은 변하지 않는다) 직원이지. 자네보다 오래 다녔네. (사이) 위에는 아직도 물류 시스템을 만드나.

**김신입** ...아틀라스요? 그걸 어떻게—

**정전무** (처음으로 타자를 멈춘다) 이름은 바뀌는군. 우리 때는 헤르메스였네. 짐을 나르는 신. (혼잣말처럼) 아틀라스라. 이제는 아예 짊어지는 신인가. 정직해서 좋군.
...
> C:\HERMES\BUILD> 자네 것이나 끝내게. 
> C:\HERMES\BUILD> 위에서 찾는군._

시스템: 도감 기록 #1997-01 『헤르메스 프로젝트』 획득 (문서 열람: 로비 메뉴)
...
【화면】 전원 야근 중. 최종 빌드 업로드 바 99%. 차만수는 넥타이를 머리에 묶었고, 백은영은 처음으로 재킷을 벗었다.

**한지우** (모니터에서 눈을 떼지 않고) ...이상해요. 코어 라이브러리에 서명 안 된 모듈이 있어요. 작성일이... 1997년 12월 24일?

**구본철** 뭐? 우리 회사 설립이 12년 전인데?

**한지우** 오픈소스 타고 들어온 거예요. 아틀라스 물류 코어의 최심부가... 이 모듈 위에서 돌아요. 30년 가까이. 작성자 주석은 지워져 있고요.

【SFX】 21:57 — 초침 역회전 3회. 전 모니터 1프레임 껌뻑.
【화면】 이번엔 아무도 사라지지 않는다. 대신 사무실 전체 모니터에 초록 텍스트가 덮인다:

> C:\HERMES\BUILD> 컴파일 중... 99%

**차만수** 뭐야, 해킹이야?! 백 실장!

**백은영** (조용히) ...이 폰트, 어디서 봤는데.
...
> 납품 빌드 최종 확정. 서명 없는 모듈을 어떻게 처리하시겠습니까?

▶ 최종 선택지
1. 「그대로 납품 버튼을 누른다」 → 엔딩 분기: 납품 성공 티어 1~3 (진척도·클라이언트 신뢰도에 따라). 정전무는 성불하지 못한다. 에필로그 마지막 컷에 지하 CRT의 97% 커서가 여전히 깜빡인다.
2. 「모듈을 최신 코드로 교체한다」 → 22:40 전체 빌드 붕괴. 배드엔딩 「토대를 뽑은 자」: 30년 묵은 토대를 뽑은 아틀라스는 서지 못한다.
3. 「백업 테이프의 원본 주석을 복원하고, 크레딧에 이름을 올려 납품한다」 (해금 조건 충족 시에만 표시) → **D-3 진행**
...
> // HERMES CORE v1.0
> // 작성: 정대웅 (한밭정보시스템 전산개발부)
> // 1997.12.24. 눈 오려나.

**정전무** (의자에 앉아 있다. 처음으로 얼굴이 보인다 — 그냥, 피곤한 40대 회사원의 얼굴) ...자네. 그거 내 건데.

**김신입** 압니다. 그래서 이름 붙여서 납품하려고요. 아틀라스 크레딧에요. 30년 동안 전 세계 물류가 대웅 님 코드 위로 지나갔어요. 그걸 이제 다들 알게 될 거예요.
...
【SFX】 전화벨.
【화면】 차만수가 수화기를 든다. 전원 정지 화면. 3초.

**차만수** (수화기를 내려놓고, 아주 작게) ...검수 통과랍니다.

【화면】 환호 대신 — 전원이 각자 자리에 그대로 주저앉는다. 한지우가 이어폰을 뺀다. 구본철이 집에 전화를 건다. 백은영이 처음 웃는다.

**김신입** (독백) 창밖에 눈이 왔다. 나는 그걸, 누군가의 몫까지 확인했다.

엔딩 타이틀: **진엔딩 — 1997의 성불**
【화면】 크레딧 롤 최상단: 「HERMES CORE — 정대웅 (1997)」
...
> **상황**: 팀 야식 5인분, 합계 61,500원. 뒤에서 구본철 팀장이 지갑을 꺼내는 소리가 들린다.
> ▶ 「팀장님 카드를 못 본 척한다」 (예산 대신 대표 신뢰도 소모 각오)
> ▶ 「내 카드로 긁는다」 (개인 자금 -61,500)
> ▶ 「주문을 라면 5개로 바꾼다」
> **결과(1 선택 시)**: 구본철이 말없이 카드를 도로 넣었고, 다음 날 차만수가 "누가 회식했어?"라고 물었다.
...
| 전임자(前任者) | 하급 | 퇴사자의 그림자 | 낮의 행정 용어를 그대로 괴이명으로 — 단어 오염 조항의 대표 사례. |
| 사이층(層) | 중급 | 존재하지 않는 4층 엘리베이터 | 3층과 5층 '사이'. 버튼에는 없는데 멈추는 층. |
| 무한연장(無限延長) | 중급 | 회의실 | 끝나지 않는 회의. 들어간 사람의 체감 시간을 늘린다. |
| 정전무(停電無) | 상급 | 지하 서버실 / 1997년 | 직급(전무)으로 들리지만 실제 뜻은 '정전이 없다' — 만년 대리의 별명이라는 반전까지 이름 하나에 담는다. |
...
| 아이템 | 플레이버 텍스트 |
|---|---|
| 골드라벨 믹스커피 | 12년째 리뉴얼 예정. 맛은 그대로라 다행이다. |
| 법인카드 | 한도는 대표의 기분과 연동된다. 공식 사양은 아니다. |
| 탄력근무 신청서 | 결재란이 5칸이다. 탄력적인 건 결재 속도뿐. |
| 인체공학 의자 | 인체공학적으로 설계됐다. 12시간 앉는 인체는 고려 밖. |
| 종이컵 탑(17개) | 탕비실 지층에서 발굴. 층마다 야근의 연대가 다르다. |
...
대상 5인: 김신입(플레이어), 차만수, 백은영, 구본철, 한지우.
...
**캐릭터 파트 총계: 초상 32장 + 차분 40세트 + 스프라이트 284프레임.**
...
| 분류 | 수량 | 규격 |
|---|---|---|
| 픽업 아이템 아이콘 30종 (믹스커피 박스, 에너지드링크, 초코파이, 컵라면, 법인카드, USB, 노트북 충전기, 담요, 소화기, 부적, 소금, 손전등, 상비약, 박카스, 인감도장, 이력서(한지우), 골프공(차만수), 결재판, 멀티탭, 랜케이블, 외장하드, 마우스, 텀블러, 핫팩, 안대, 이어플러그, 명함, 사원증, 라이터, 빛바랜 플로피디스크) | 30 | 128×128, 낮 팔레트 단일(인벤 UI는 LUT 제외) |
...
| 카테고리 | S | A | B | 산출 |
|---|---|---|---|---|
| 현실 이벤트 25종 | 6 (★대표의 골프 회식, ★클라이언트 현장 급습, 한지우 면접 목격, 구본철 가정 전화, 백은영 정치 제안, 법인카드 정지) | 9종 → 베이스 3장 + 파츠 18개 | 10종 → 아이콘 10장 | 풀 6장 + 베이스 3장 + 파츠 18 + 아이콘 10 |
...
| 엔딩 15종 | 6 (★납품 성공 1티어, ★부도, 진엔딩 '1997의 성불', 판타지 엔딩, 전원 퇴사, 김신입 실종) | 6종 → 베이스 2장 + 파츠 12개 | 3종 → 아이콘 3장 | 풀 6장 + 베이스 2장 + 파츠 12 + 아이콘 3 |
...
| BGM-10 | 납품의 아침 | 성공 엔딩, 브라스 팡파레 | 2:20 | × |
...
| SFX-039 | 골프 연습 스윙 | 차만수 이벤트 | 퍼터+카펫 |
...
| SFX-061 | ST 납품 성공 | 시그니처 연출④ | 팡파레 프리롤 |
...
| 6.0–8.0 | "검수 통과" 메일 팝업 | SFX-041 3연타 → SFX-061 |
...
| 5.0–7.5 | 어둠 속 "㈜버티고" 현판에 압류 딱지 | SFX-062 셔터음 |
...
| 초상화·감정 차분 | 외주(캐릭터 일러스트레이터 1인, 단가 통일 위해 단일 작가) | 멘탈 ①③⑤ 3단계 ×5인=15장 + 차분 각 4종 | 잔여 단계 10장 + 차분 잔여 20세트 + 괴이 초상 7장 |
...
| 인물 id | `ch_daepyo`(차만수) `ch_siljang`(백은영) `ch_timjang`(구본철) `ch_daeri`(한지우) `ch_player`(김신입) |
...
| 인물 스탯 | `mental.<인물>` `hp.<인물>` | `mental.한지우<30` |
| 상태이상 | `status.<인물>.<상태id>` (bool) | `status.구본철.빙의` |
| 플래그 | `flag.<이름>` (bool) | `!flag.jiwoo_interview` |
...
```json
{
  "id": "ev_jiwoo_secret_interview",
  "phase": "event",
  "pool": "day",
  "condition": "day>=3 && mental.한지우<30 && !flag.jiwoo_interview",
  "weight": 18,
  "weightMods": [{ "if": "trust<20", "mul": 1.5 }],
  "once": true,
  "cooldownDays": 0,
  "tags": ["jiwoo_arc", "hr"],
  "titleKey": "ev_jiwoo_secret_interview.title",
  "bodyKey": "ev_jiwoo_secret_interview.body",
  "choices": [
    {
      "textKey": "ev_jiwoo_secret_interview.c1",
      "require": "budget>=50000",
      "outcomes": [
        { "weight": 70, "textKey": "ev_jiwoo_secret_interview.c1.ok",
          "effects": [
            { "op": "stat", "target": "mental.한지우", "delta": 15 },
            { "op": "stat", "target": "budget", "delta": -50000 },
            { "op": "flag", "key": "jiwoo_interview", "value": true },
            { "op": "chain", "chainId": "chain_jiwoo_arc", "eventId": "ev_jiwoo_stays", "delayDays": 2, "expiresInDays": 3 }
          ] },
        { "weight": 30, "textKey": "ev_jiwoo_secret_interview.c1.fail",
          "effects": [ { "op": "status", "target": "ch_daeri", "add": "번아웃" } ] }
      ]
    },
    {
      "textKey": "ev_jiwoo_secret_interview.c2",
      "require": null,
      "outcomes": [
        { "weight": 100, "textKey": "ev_jiwoo_secret_interview.c2.ok",
          "effects": [ { "op": "flag", "key": "jiwoo_interview", "value": true },
...
```json
{
  "id": "ch_daeri", "nameKey": "ch_daeri.name",
  "base": { "mental": 35, "hp": 70 },
  "aptitude": { "dev": 9, "doc": 6, "qa": 8, "client": 4 },
  "traits": ["burnout_prone", "job_hunting"],
  "statusImmune": [],
  "restRecovery": { "mental": 12, "hp": 8 },
  "barks": { "lowMental": "ch_daeri.bark.low", "assigned": "ch_daeri.bark.work" },
  "arcFlags": ["jiwoo_interview", "jiwoo_stays", "jiwoo_quit"]
}
```
...
```json
{
  ...
  "chars": { "ch_daeri": { "mental": 24, "hp": 61, "status": ["번아웃"] } },
  "inventory": [ { "id": "it_redtaurine", "n": 2 }, { "id": "it_talisman_post_it", "n": 1 } ],
  "relics": ["rl_id_card_1997"],
  "flags": { "jiwoo_interview": true },
  "seenOnce": ["ev_jiwoo_secret_interview"],
  "lastSeen": { "ev_boss_golf": 2 },
  "chainQueue": [ { "chainId": "chain_jiwoo_arc", "eventId": "ev_jiwoo_stays", "fireDay": 6, "expiresDay": 7 } ],
...
|---|---|---|---|
| **60 Seconds!** (Robot Gentleman, 2015) | 60초 스캐빈지 → 일차별 텍스트 생존 | 스크램블의 아케이드성, 일차 결산의 긴장 리듬, 아이템 조합 이벤트 해금 구조 | 생존 대상이 '핵전쟁'이 아니라 '납품 D-7' — 벙커 대신 사무실, 가족 대신 직장 동료. 밤 페이즈(이면 오피스)라는 제3의 레이어 추가. **에셋·명칭·UI 레이아웃은 전면 오리지널**(8.5 참조) |
...
- [ ] 사내 고유명사('제왕그룹', '㈜버티고', '아틀라스', '골드라벨') 실존 기업·상품명 충돌 검색 — 충돌 시 즉시 개명 (특히 '아틀라스'는 실존 물류·지도 서비스 다수, "아틀라스 v7" 등 프로젝트 코드명 형태로만 노출)
- [ ] 폰트: 유료 라이선스 2종(본문·호러체) 재배포 조건 확인, 무료 폰트는 OFL만 사용
- [ ] 사운드: 전 트랙 자체 제작 또는 매절 계약, CC 라이선스 에셋 사용 금지
- [ ] 실존 인물·기업 풍자 리스크: 차만수 등 4인 캐릭터의 실존 인물 유사성 내부 리뷰 (특정 기업 오너 연상 요소 제거)
...
| 5 | `assign_commit` | 업무 배치 확정 | char_id→slot_id 매핑 4쌍 | 배치 메타 고착 여부 (한지우 개발 몰빵률 등) |
...
| 아무 엔딩 도달률 | **60%** | 납품 성공 티어 1회 이상 경험 유저 비율 | 진척도 곡선 완화 |
...
- [ ] 상호 배타 이벤트(한지우 이직 발각 ↔ 이직 은폐 성공)의 동시 발동 불가 검증
...
| `stat <char> <mental\|hp> <n>` | `stat hanjw mental 5` | 인물 수치 설정 (char: chams/baeky/gubc/hanjw) |
| `status <char> <id> [clear]` | `status gubc possessed` | 상태이상 부여/해제 |
...
| 2 | **"우리 회사에도 있는 사람"** | 4인방 성격 유형 테스트 웹 페이지(데모 유입용). "당신의 직장 상사는 차만수형? 백은영형?" 결과를 데모 링크와 함께 공유 |
